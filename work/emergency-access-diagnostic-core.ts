...
export async function captureEmergencyAccessDiagnostic(tokens: TokenSource, input: DiagnosticInput): Promise<EmergencyDiagnosticArtifact> {
  ...
  const roleSchedulesRaw = await paged(tokens, 'role-schedules', `${V1}/roleManagement/directory/roleAssignmentScheduleInstances?$select=id,principalId,roleDefinitionId,directoryScopeId,assignmentType,startDateTime,endDateTime`, ['id', 'principalId', 'roleDefinitionId', 'directoryScopeId', 'assignmentType', 'startDateTime', 'endDateTime'], requests, ['id', 'principalId', 'roleDefinitionId', 'directoryScopeId', 'assignmentType'])
...
function preparedAccount(artifact: EmergencyDiagnosticArtifact, accountId: string): string[] {
  ...
  const permanentSchedule = rows(artifact.roleSchedules).some(role => role.principalId === accountId && role.roleDefinitionId === GLOBAL_ADMIN && role.directoryScopeId === '/' && role.assignmentType === 'Assigned' && role.startDateTimeKnown === true && (role.startDateTime === null || beforeOrAt(role.startDateTime, captureAt)) && role.endDateTimeKnown === true && role.endDateTime === null)
