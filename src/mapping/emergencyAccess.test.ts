...
test('the recommending name is a purpose phrase the tenant wrote, bounded at both ends, never a word inside a person’s name', () => {
  // The name signal recommends on its own, so it reads a purpose phrase the
  // tenant wrote, never a word that happens to appear in somebody's name. The
  // phrase has to be the whole token at both ends (account numbering aside):
  // without both boundaries a word that merely contains it would classify, and
  // an account so named would leave the people population with nobody deciding.
  const names = [
    'Breakglass', 'Break Glass', 'Break-Glass', 'break_glass', 'Break Glass 2', 'Break-glass 01', 'breakglass2', 'breakglass@contoso.com',
    'BreakGlass Admin', 'Emergency Access', 'Emergency-Access', 'EmergencyAccess', 'emergency access 1', 'emergencyaccess01', 'emergency admin',
    'Emergency Account', 'Emergency Administrator', 'Emergency Access (do not disable)', 'svc-breakglass', 'svc_emergency-access', 'IT.breakglass',
    'BG-Admin', 'bg_admin', 'BG Admin', 'contoso bg', 'bg1', 'bg01',
  ]
  const notNames = [
    'Alice Glass', 'John Glassman', 'Glass, Alice', 'Breakwater', 'Breakfast Club', 'Kim Breakwell', 'Glasscock Holdings', 'Bigby Wolf', 'Bridget Gallagher', 'Sam Lee',
    'Unbreakglass', 'NonEmergencyAccess', 'Xbreakglass', 'unbg', 'prebreak glass', 'Breakglassman', 'Breakglassware', 'breakglass2man',
    'Emergency Accessory', 'Emergency Administratorial', 'Emergency AdminAssistant', 'Emergency Accountancy', 'Emergency Services Liaison', 'Emergency Contact Mailbox',
    'bgood', 'bgood@contoso.com', 'BGuard',
  ]
  for (const name of names) assert.equal(isEmergencyName(name), true, `${name} names the account for the job`)
  for (const name of notNames) assert.equal(isEmergencyName(name), false, `${name} is a name, not a purpose`)
})

test('the name nominates and recommends on its own; weak signals only nominate, together; guests and disabled accounts never; none found is empty', () => {
  // a person whose surname is Glass keeps every weak signal and is still not recommended
  {
  ...
  // the name nominates on its own, two weak signals nominate together; guests and disabled accounts never do
  {
    const s = fixtureSnapshot()
  ...
  // weak signals never recommend: a Global Administrator that is unlicensed and .onmicrosoft.com stays a person
  {
    const s = fixtureSnapshot()
    s.config.caPolicies!.rows = []
  ...
  // none found is an empty list, not an error
  {
    const s = fixtureSnapshot()
    s.roles = { active: {}, eligible: {} }
...
test('detection never confirms: on every scan, whoever is signed in, in any order, nobody leaves the people population until a person chooses', () => {
  // a fresh scan of a four-account tenant confirms nothing: 3 active people, 0 emergency access, 1 sign-in disabled
  ...
  // an obvious Breakglass account is recommended on every scan and silently confirmed on none of them
  ...
  // the ledger is the same whoever is signed in: the operator changes no classification
  ...
  // two plausible candidates: whatever order they enumerate in, nothing is chosen
...
test('a saved decision is the authority, and a re-scan neither adds to it nor drops from it', () => {
  // a saved decision is the authority: the accounts a person picked are the emergency accounts
  ...
  // a re-scan keeps the operator’s choice exactly: nothing added, nothing dropped
...
test('a legacy record or a plan file\'s mapping keeps its emergency ids as prior context; only the saved decision confirms', () => {
  // a legacy record’s emergency ids are kept as prior context and re-asked, never inherited as a confirmation
  ...
  // a plan file round-trip: the mapping’s ids are prior context, the saved decision is the authority
