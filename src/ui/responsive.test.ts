// Cross-surface responsive conformance (task 039).
//
...
//
// The first is that each surface still uses ITS OWN breakpoints. The four packs
// do not share a responsive system — Home turns at 760 and 560, Connect at 760
// alone, Plan at 940 and 650, MFA Readiness at 900 and 620 — and the cheapest
// wrong answer to "make the product responsive" is one shared mobile width that
// flattens all four into the same stack. So the breakpoints below are READ OUT
// OF THE CANONICAL FILES at test time rather than typed here: if a pack's bytes
// ever change, this test changes with it, and if production quietly normalises
// a surface onto another surface's width, it fails.
//
...
function mediaBody(css: string, width: number): string {
  const open = css.indexOf(`@media (max-width: ${width}px)`)
...
test('each pack still declares the breakpoints production is built against', () => {
  // The pack is the authority for its own responsive model. These are the
  // numbers the rest of this file uses, and reading them here means a change to
  // an approved file cannot pass silently as "production is still conformant".
  ...
  assert.deepEqual(breakpoints(read(PACKS.mfa)), [900, 620])
})
...
test('no surface is normalised onto another surface\'s breakpoint', () => {
  ...
  // carry Connect's, Plan's and MFA's five distinct widths at once.
  assert.deepEqual(breakpoints(HOME_CSS), [760, 560])
  for (const w of [940, 900, 760, 650, 620]) {
...
test('MFA Readiness steps its summary through the pack\'s two states', () => {
  const at900 = mediaBody(APP, 900)
  const at620 = mediaBody(APP, 620)
  // Four cells become two columns with the dominant cell spanning both …
  assert.match(at900, /\.readiness-summary\s*\{[^}]*grid-template-columns:\s*1fr 1fr/, 'the summary does not become two columns')
  assert.match(at900, /\.summary-main\s*\{[^}]*grid-column:\s*1 \/ -1/, 'the dominant summary cell does not span')
  // … and then one, with the dominant cell giving up its span.
  assert.match(at620, /\.readiness-summary\s*\{[^}]*grid-template-columns:\s*1fr/, 'the summary does not become one column')
  assert.match(at620, /\.summary-main\s*\{[^}]*grid-column:\s*auto/, 'the dominant cell keeps a span it no longer has columns for')
  // The counts are cells of one panel, not tiles that float away from it.
  assert.doesNotMatch(at900, /\.summary-stat\s*\{[^}]*border-radius/)
  assert.doesNotMatch(at620, /\.summary-stat\s*\{[^}]*display:\s*none/, 'a count is dropped on a phone')
})

test('MFA Readiness stacks a person row without losing a field or its label', () => {
  const at900 = mediaBody(APP, 900)
  assert.match(at900, /table\.datatable,[\s\S]*?tr,[\s\S]*?td\s*\{\s*display: block/, 'the person row does not stack')
  ...
  assert.match(at900, /thead\s*\{[^}]*clip-path:\s*inset\(50%\)/, 'the table head is not the visually-hidden pattern')
  assert.doesNotMatch(at900, /thead\s*\{[^}]*display:\s*none/, 'the table head is removed from the accessibility tree')
  ...
  assert.match(at900, /\.cell-key\s*\{\s*display:\s*block/, 'the stacked field label never appears')
...
test('MFA Readiness tightens its gutter at the pack\'s 620', () => {
  const at620 = mediaBody(APP, 620)
  for (const part of ['header.app', '> main.page', 'footer.app']) {
    assert.ok(at620.includes(`.shell[data-route='readiness'] ${part}`), `${part} does not tighten with the readiness page`)
  }
  assert.match(at620, /padding-left:\s*12px/, 'the readiness gutter does not tighten')
  assert.match(at620, /\.callout-body\s*\{[^}]*flex-direction:\s*column/, 'the callout does not stack its explanation above its action')
})
...
test('no narrow rule hides a control, a blocker or a state word', () => {
  ...
  const CANNOT_VANISH = ['.btn', '.blocking', '.callout', '.status', '.decision', '.tenant-object', '.stage-label', '.cell-key', '.step-side', '.connect-step-actions']
  for (const w of [940, 900, 760, 650, 620]) {
    const body = mediaBody(APP, w)
