...
test('tile 2, Baseline: the nested card carries name, size and version; the source disclosure claims only the provenance it has; the author-update rows; an author check that could not run never reads as no update', () => {
...
test('each Scan state renders only its own strings, and each action names what it does', () => {
  {
...
test('a complete scan stays done and lists the sections it did not read in full, as phrases, the directory audit read included', () => {
...
test('the ask and another account follow the roles read: no ask for a role held or not yet known, and another account only where it can unblock the plan', () => {
...
test('each Plan state renders only its own strings: ready with its counts, the last full plan with its date, waiting with nothing to press', () => {
  {
...
test("the strip reads the stages: the complete scan's own caveat, and the same load in flight as the baseline step", () => {
