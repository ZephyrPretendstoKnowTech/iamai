...
const contentStep = read('src/ui/surfaces/ContentStep.tsx') + read('src/ui/surfaces/stepBody.ts')
...
test("the Plan step's implementation channels are one tab set over one panel, and each is offered only where it exists", () => {
  ...
  assert.match(contentStep, /const channels = step\.directionQuestions \? \[\x27ai\x27 as Channel\] : deployNow \? channelsFor\(hasPortal, contract\.implementation\.offered\) : \[\]/)
