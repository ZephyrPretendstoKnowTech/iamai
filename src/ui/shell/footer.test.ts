...
test('"people" on MFA Readiness, the Plan and Connect; "user" only for an Entra user object', () => {
  ...
  // The portal's own labels name the Users condition and User risk as Microsoft does,
  // and the six guest types a policy can include by Microsoft's names for them
  // (the drift names and "Add {group} under Users → Exclude.").
  ...
  const words = JSON.stringify(strings({ readiness: pages.readiness, plan: pages.plan, connect: pages.connect, appReadiness: app.readiness, appPlan: app.plan, appConnect: app.connect })).replaceAll(JSON.stringify((pages.plan as { impact: { noUserImpact: string } }).impact.noUserImpact), '""').replace(/All users/g, 'Everyone').replace(/\bUsers → /g, '→ ').replace(/"Users"/g, '""').replace(/User risk/g, 'risk').replace(/Per-user MFA/g, 'MFA').replace(/user accounts/g, 'accounts').replace(/(local guest|B2B collaboration guest|B2B collaboration member|B2B direct connect|other external|service provider) users/gi, 'guests')
