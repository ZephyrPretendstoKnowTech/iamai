...
test('the in-step scan ends at the step: the demo advances to week two and the countries step reopens', () => {
  ...
  assert.equal(afterScanHref(null), PLAN_HREF, "the header's scan lands on the Plan")
  assert.equal(afterScanHref('#/connect'), PLAN_HREF, 'anything but a step link lands on the Plan')
