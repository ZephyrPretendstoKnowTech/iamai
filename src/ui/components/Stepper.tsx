...
const CHIP: Record<StepperStatus, { status: ChipStatus; text: string } | null> = {
  notStarted: null,
  inProgress: { status: 'in-progress', text: 'in progress' },
  done: { status: 'done', text: 'done' },
  attention: { status: 'warning', text: 'needs attention' },
}
...
export function Stepper({
  ...
  return (
    <nav className="stepper" aria-label="Steps">
      <div className="stepper-group-title">Steps</div>
      {steps.map((s, i) => {
      ...
      <div className="stepper-group-title">Reference</div>
      {reference.map((s) => (
