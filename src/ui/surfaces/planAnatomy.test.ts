...
test('Implementation is the pack’s pill channels over a fixed preview, or one truthful no-action box', () => {
  ...
  // With the board's route start the export view reads (stepExport.ts stepExportView).
  assert.match(CONTENT_STEP, /stepContext\(step, \(s\) => stepExportView\(s, ctx, laneView(?:, prerequisiteLabel\?\.startOf)?\)\)/, 'AI Info is not the step context the prompts ground themselves in')
...
function stateOf(o: {
  ...
  const c = {
    ...
    // The turn-on waits are the contract's own list (R4-31), which the Readiness
    // cards read: none of the five canonical states holds one.
    enforcementWaits: [],
    doneWhen: ['Done.'],
    members: [],
    multiPolicy: false,
