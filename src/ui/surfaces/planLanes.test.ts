...
test('every row lands in exactly one lane, delivered work is Completed and skipped work is Deferred', () => {
  ...
  for (const make of RUNS) {
    ...
    for (const s of r.steps) {
      ...
      // Delivered work with a conditional input nobody saved waits On Hold on the Direction step that asks it until the Save, never Completed and never Observing (U28, B10 P0-12; walk list 4.x item 6).
