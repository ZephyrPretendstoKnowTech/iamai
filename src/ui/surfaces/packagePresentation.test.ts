...
// a source conflict with its package set aside for review still says so, names
// the pins, and says why nothing is implemented; the AI warning every package
// repeated is said once, by the runtime.
import { test } from 'node:test'
import assert from 'node:assert/strict'
import { readFileSync } from 'node:fs'
import registry from '../../content/implementation/registry.generated.json' with { type: 'json' }
import type { CompiledPackage } from '../../content/implementation/protocol.ts'
import { NO_RUNTIME, projectImplementation } from '../../content/implementation/project.ts'
...
test('the Admin Portal source conflict shows its review, its pins, the conflict and why nothing is implemented, and resolves nothing', () => {
  for (const name of ['demo', 'demo-week2'] as const) {
    ...
    assert.match(line, new RegExp(`${String(reviewed.meta.baselineAuthority?.pinCommit).slice(0, 8)}.*${BASELINE_COMMIT.slice(0, 8)}`), line)
  }
  ...
  assert.match(empty, /withheld\.map/, 'the review note is drawn only beside artifacts')
