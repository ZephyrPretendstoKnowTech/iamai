...
import { validCompletionDate, RECOVERY_PREPARATION_WORKFLOW } from '../../roadmap/cleanupDone.ts'
...
export function RecoveryTestControl({ phase, purpose, onDone }: { phase: CleanupPhase; purpose: RecoveryPurpose; onDone: RecoveryDone }) {
  ...
  const failedAfter = purpose === 'final' ? phase.latestFailedAtByAccount : undefined
  const preparationCurrent = phase.accountIds.length > 0 && phase.accountIds.every(id => !!preparedAt?.[id])
  const confirmation = purpose === 'final' ? phase.recoveryFindings?.find(finding => finding.key === 'recovery-confirmation') : null
  const resultCurrent = (id: string): boolean => confirmation?.items?.some(item => (item.accountId === id || item.subjectId === id) && item.outcome === 'pass') === true
  ...
  const startReason = !phase.accountIds.length ? 'Select and save emergency accounts first.' : !configurationReady ? 'Complete the highlighted configuration tasks before starting verification.' : !phase.tenantId || !phase.snapshotObservedAt ? 'Scan the tenant before starting verification.' : phase.accountIds.every(resultCurrent) ? 'The selected accounts have current verification.' : preparationCurrent ? 'Configuration is recorded. Perform the sign-in, then scan again.' : null
  const evidence = (): Record<string, VerifiedRecoveryEvidence> => Object.fromEntries(tested.flatMap(id => {
    const reading = (candidates?.[id] ?? []).find(item => item.qualifies && item.candidate.eventId === selectedEvents[id] && (!failedAfter?.[id] || Date.parse(item.candidate.at) > Date.parse(failedAfter[id]!)))
  ...
  const failureDateValid = validCompletionDate(failedOn, new Date().toISOString(), Intl.DateTimeFormat().resolvedOptions().timeZone)
  const saveReason = !outcome ? 'Choose a verification result.' : !tested.length ? 'Select at least one tested account.' : outcome === 'failed' && !failureDateValid ? 'Enter a valid attempt date that is not in the future.' : outcome === 'passed' && !passReady ? 'Choose a qualifying event and confirm both checks for every selected account.' : null
  const save = (): void => {
    if (!outcome || !tested.length) return
    if (outcome === 'failed') { if (!failureDateValid) return; onDone(failedOn, tested, { outcome, purpose }); return }
  ...
  return <div className="recovery-test-control">
    ...
    <Button variant="secondary" disabled={!configurationReady || preparationCurrent || !phase.tenantId || !phase.snapshotObservedAt} onClick={() => onDone(today(), phase.accountIds, { workflow: RECOVERY_PREPARATION_WORKFLOW, purpose, tenantId: phase.tenantId, configurationObservedAt: phase.snapshotObservedAt })}>Start verification</Button>
    <p className="reason">{startReason ?? 'Record the current configuration before the sign-in. An older event cannot certify a later checkpoint.'}</p>
    <fieldset><legend>Accounts tested</legend>{phase.accountIds.length === 0 ? <p>Select and save an emergency account first.</p> : phase.accountIds.map((id) => {
      const readings = candidates?.[id] ?? []
      const qualifying = preparedAt?.[id] ? readings.filter(reading => reading.qualifies && (!failedAfter?.[id] || Date.parse(reading.candidate.at) > Date.parse(failedAfter[id]!))) : []
      const label = phase.accountUpnsById?.[id] ?? id
      const current = resultCurrent(id)
      return <div key={id} className="decision-fields recovery-account-choice"><label className="option-value"><input type="checkbox" checked={tested.includes(id)} onChange={event => { const checked = event.currentTarget.checked; setTested(ids => checked ? [...new Set([...ids, id])] : ids.filter(item => item !== id)); if (!checked) { setSelectedEvents(values => { const next = { ...values }; delete next[id]; return next }); setCredentialConfirmed(values => ({ ...values, [id]: false })); setRecoveryConfirmed(values => ({ ...values, [id]: false })) } }} /><span>{label}{current ? ' · Current verification recorded' : ''}</span></label>{tested.includes(id) && outcome !== 'failed' && <>
        ...
        {qualifying.length === 0 && <p className="reason">{!preparedAt?.[id] ? 'Start verification first.' : readings[0]?.reason ?? 'No qualifying fresh administrative passkey sign-in was found. Sign in, then scan again.'}</p>}
    ...
    {outcome === 'failed' && <label><strong>Attempt date</strong><input type="date" max={today()} value={failedOn} onInput={event => setFailedOn(event.currentTarget.value)} onChange={event => { const value = event.currentTarget.value; setFailedOn(value) }} /></label>}
    <Button variant="secondary" disabled={saveReason !== null} onClick={save}>{outcome === 'failed' ? 'Record failed attempt' : 'Save verification'}</Button>
    {saveReason && <p className="reason">{saveReason}</p>}
  </div>
}
