...
export function RecoveryTestControl({ phase, purpose, onDone }: { phase: CleanupPhase; purpose: RecoveryPurpose; onDone: RecoveryDone }) {
  ...
  const startReason = !phase.accountIds.length ? 'Select and save emergency accounts first.' : !configurationReady ? 'Complete the highlighted configuration tasks before starting verification.' : !phase.tenantId || !phase.snapshotObservedAt ? 'Scan the tenant before starting verification.' : preparationCurrent ? 'Configuration is recorded. Perform the sign-in, then scan again.' : null
  ...
  const saveReason = !outcome ? 'Choose a verification result.' : !tested.length ? 'Select at least one tested account.' : outcome === 'passed' && !passReady ? 'Choose a qualifying event and confirm both checks for every selected account.' : null
  const save = (): void => {
  ...
  return <div className="recovery-test-control">
    ...
    <fieldset><legend>Accounts tested</legend>{phase.accountIds.length === 0 ? <p>Select and save an emergency account first.</p> : phase.accountIds.map((id) => {
      ...
      return <div key={id} className="decision-fields recovery-account-choice"><label className="option-value"><input type="checkbox" checked={tested.includes(id)} onChange={event => { const checked = event.currentTarget.checked; setTested(ids => checked ? [...new Set([...ids, id])] : ids.filter(item => item !== id)); if (!checked) { setSelectedEvents(values => { const next = { ...values }; delete next[id]; return next }); setCredentialConfirmed(values => ({ ...values, [id]: false })); setRecoveryConfirmed(values => ({ ...values, [id]: false })) } }} /><span>{label}</span></label>{tested.includes(id) && <>
    ...
    <Button variant="secondary" disabled={!outcome || !tested.length || outcome === 'passed' && !passReady} onClick={save}>{outcome === 'failed' ? 'Record failed attempt' : 'Save verification'}</Button>
