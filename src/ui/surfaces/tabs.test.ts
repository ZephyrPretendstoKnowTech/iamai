...
test('for every policy step, the three Do it tabs differ and the PowerShell carries the JSON tab\'s displayName', () => {
  ...
  const two = powershellFor([
    { sourceName: 'A', memberKey: 'A', mode: 'create', policyId: null, body: { displayName: 'A', state: 'enabled', conditions: { users: { includeUsers: ['All'] }, applications: { includeApplications: ['All'] } }, grantControls: { operator: 'OR', builtInControls: ['mfa'] } } },
    { sourceName: 'B', memberKey: 'B', mode: 'create', policyId: null, body: { displayName: 'B', state: 'enabled', conditions: { users: { includeUsers: ['All'] }, applications: { includeApplications: ['All'] } }, grantControls: { operator: 'OR', builtInControls: ['mfa'] } } },
  ])
