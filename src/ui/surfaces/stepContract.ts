...
export function readinessOf(step: Step, c: StepContract, blockers: readonly PrerequisiteBlocker[] = [], prerequisiteLabel: PrerequisiteLabel = () => null, o: { setupAfterEnforcement?: boolean } = {}): ContractReadiness {
  ...
  const facts = [enforcedReadingTile(step), unwatchedTile(step), ownPolicyTile(step), followUpTile(c), ...emergencyTiles(step, c), ...configuredTiles, ...((configuration.length && step.id !== 's-prereq-break-glass') || (c.satisfiedFacts?.length ?? 0) > 0 ? [] : [stateTile(step, c, o.setupAfterEnforcement === true)]), dormantTile(c), exclusionsTile(step, c), exclusionsReachTile(c), implementationTile(step, c), inventory].filter((x): x is ReadinessTile => x !== null)
  ...
  const present = new Set<string>([...facts.map((t) => t.key), ...fixes.map((t) => t.key), ...waits.map((t) => t.key), ...(operator ? [operator.key] : [])])
  ...
  const tiles = directOnly([...lead, ...unsavedTiles(step), ...fixes.filter(own), ...waits.filter(own), ...(operator ? [operator] : []), ...engineTiles(c, effectiveBlockers, present, prerequisiteLabel)], rowNamed)
