...
export function stepVars(step: Step, ctx: StepVarContext): Record<string, unknown> {
  ...
  Object.assign(v, contentLists({ snapshot: ctx.snapshot, mapping: ctx.mapping, nameOf: ctx.nameOf, now: ctx.now, operatorId: ctx.operatorId, mfaInPlace: ctx.mfaInPlace === true }))
