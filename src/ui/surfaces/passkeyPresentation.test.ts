...
test('the passkey-only presentation explains the model controls and uses scoped readable layouts', () => {
  ...
  assert.match(css, /\.step:is\([^}]*s-prereq-passkey-settings[^}]*\) \.readiness-strip\s*\{\s*grid-template-columns: repeat\(2, minmax\(0, 1fr\)\)/)
  assert.match(css, /@media \(max-width: 640px\)[\s\S]*\.step:is\([^}]*s-prereq-passkey-settings[^}]*\) \.readiness-strip \{ grid-template-columns: minmax\(0, 1fr\)/)
