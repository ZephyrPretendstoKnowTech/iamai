...
test('unfinished work is never dated in the past: a plan started ten working days ago places its preparation today, keeps its start, moves what follows, and a weekend read moves to the next working day', () => {
  {
    const f = inUtc('demo')
...
test('a report-only window reads its review day, or that its review is due once the day passed with no scan; a finished step keeps the day it was finished', () => {
  {
...
test('the When column never dates a row the board holds, never calls a row finished that the board does not, and a Ready · Decision row says the decision', () => {
