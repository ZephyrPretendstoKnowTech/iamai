...
// PowerShell, no download (roadmap/operations.ts implementationOffered). The
// same holds while the only thing left to submit is an enforcement the policy's
// report-only window has not earned — Foundation A's own hold. The step says
 ...
 * the tenant describes it: the line then names the strength by the id its
 * request carries, or names none where a planning preview's stand-in holds the
 * place of one still to resolve (roadmap/portalLines.ts grantLine), never by a
 * name for a different object.
 */
...
function contextFor(p: PinnedPolicy, names: PortalNames, used: StepResolution['tenant'], openName?: string): PortalContext {
  ...
  // scan has no row for — is named by its id, and a preview's strength still to
  // resolve by nothing (portalLines.ts grantLine), never by the baseline author's
