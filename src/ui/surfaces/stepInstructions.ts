...
// change is due.
//
// One authority decides, and it decides before the content does: Foundation A's
// `policyResult` (roadmap/operations.ts), in either of its two negative answers.
//
// - `unavailableReason` — the policy cannot be written at all: an object it
//   names is missing, a pair the plan cannot match, a baseline that contradicts
//   itself, a reach that cannot be shown to spare the emergency accounts, a
//   readiness gate that has not opened;
// - `policyHold` — it can be written and today is not the day: the policy is
//   deployed in report-only and the only thing left to submit is the
//   enforcement its window has not earned (`observation-incomplete`).
//
// Both are the same answer the four implementation channels read
// (`implementationOffered`), so the lines and the channels stand down together.
//
...
import { createHeldOnReadiness, policyHold, unavailableReason } from '../../roadmap/operations.ts'
