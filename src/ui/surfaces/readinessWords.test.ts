...
test('P1-4 and P1-5: Separate Accounts and Dormant Accounts offer Entra and AI Info', () => {
  ...
  for (const id of ['s-check-separate-admin-accounts', 's-check-dormant-accounts']) assert.deepEqual(channelTabsOf(demo.get(id)!.artifacts).map((t) => String(t.label)), ['Entra', 'AI Info'], id)
