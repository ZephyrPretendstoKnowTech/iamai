 ...
 */
export function policyCardsOf(contract: StepContract, projected: EmergencyTaskProjection | null, subject: string = POLICY_SUBJECT, check: string | null = null, own: OwnCard | null = null): EmergencySubjectTile[] {
 ...
 */
export function policySubjectsOf(contract: StepContract, readiness: ContractReadiness, projected: EmergencyTaskProjection | null, subject: string = POLICY_SUBJECT, check: string | null = null, own: OwnCard | null = null): EmergencySubjectTile[] {
  ...
  const cards = policyCardsOf(contract, projected, subject, check, own).map((c) => (said.has((c.detail ?? '').trim()) ? { ...c, detail: '' } : c))
  return [...cards, ...rest]
}
