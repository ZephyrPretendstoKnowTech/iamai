...
export function CleanupBody({ phase, row, status, onScan, onDone }: {
   ...
   */
  status: { word: string; tone: StatusTone; waitingFor?: string | null; held?: string | null }
  ...
  if (alerting) {
    return (
      <article className="step panel panel-key" data-step-id="cleanup-alerting" data-task-anatomy={groupOf('cleanup-alerting')?.key}>
        {head}
    ...
    return (
      ...
      <article className="step panel panel-key" data-step-id="cleanup-drill" data-task-anatomy={groupOf('cleanup-drill')?.key}>
        ...
        <div className="step-body has-rail">
          ...
          <StepActionColumn rail={{ headline: milestoneHeadlineOf(row.done ? status.word : null, [drillMilestone(row)]), instruction: null }} />
          <div className="step-main step-main-rest">
