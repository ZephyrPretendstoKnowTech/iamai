...
test('a device chip survives export as words, never a symbol',()=>{const d={os:'Windows' as const,type:'computer' as const,lastSeen:'2026-09-01T00:00:00Z',trust:'joined' as const,version:null,best:'windowsHello' as const,builtIn:true,possible:'yes' as const,whyNot:null,offer:'windowsHello' as const,proof:{cls:'windowsHello' as const,at:'2026-09-01T00:00:00Z'},covered:true,seamless:true};assert.equal(deviceChip(d,'seamless').word,'Seamless');assert.equal(deviceChip({...d,proof:null,covered:false,seamless:false},'confirm').word,'Not confirmed')})
