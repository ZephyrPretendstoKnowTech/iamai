...
test('the row label is Lane · substatus or reason, from one function', () => {
  ...
  const ready = { lane: 'Ready' as const, substatus: 'Observing' as const, reason: null, order: 0, fromEngine: true }
  ...
  const upNext = { lane: 'Up Next' as const, substatus: null, reason: blocker, order: 0, fromEngine: true }
  assert.equal(laneLabelOf(upNext, titleOf), 'Up Next · After Emergency Access Accounts')
  const held = { lane: 'On Hold' as const, substatus: null, reason: { ...blocker, kind: 'sourceMapping' as const, id: 'sourceMapping:62d67e66', abnormal: true }, order: 0, fromEngine: true }
