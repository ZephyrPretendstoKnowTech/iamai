...
test('line endings, a BOM and trailing newlines hash the same, alone or together, and real changes still change the fingerprint', () => {
  // CRLF and LF hash the same
  {
  ...
  // a lone CR hashes the same as LF
  {
    assert.equal(hash(FIXTURE.replace(/\n/g, '\r')), hash(FIXTURE))
  }
  // a UTF-8 BOM does not change the fingerprint
  {
...
test('the file list is platform independent and the fingerprint is stable across calls', () => {
  // the file list is platform independent
  {
