...
test('a plan record holds decisions only: an old per-step blob keeps its skip and loses its cached status, and a decisions record round-trips', () => {
  // a pre-50.1 record: only the skip is kept; the plan renders from the snapshot, not the cached status
  {
  ...
  // a decisions-shaped record round-trips unchanged through decisionsOf
  {
    const rec = {
