...
test('every step the engine can build is listed by a section, so none reaches Ongoing by accident', () => {
  ...
  // The service goals answered No are built by goal (generate.ts: Inforcer, and Jon's AVD and SharePoint blocks since Phase 2b).
  named.push(...['inforcer-mfa', 'avd-trusted-network', 'sharepoint-trusted-network'].map(stepIdForGoal))
