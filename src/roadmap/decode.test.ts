...
/** The circumstances the records answer: who was seen on the old protocols, in the two flows, and at each risk level. */
const RECORDS = {
...
test('a submitted body carries only values Conditional Access has, checked to their nested values: scopes that scope, filters that filter, strengths by reference, session controls that say what they do', () => {
  // a submitted body is checked to its nested values, not only its keys
  {
  ...
  // a scope has to scope somebody, and a filter has to filter
  {
  ...
  // the strength IAMAI submits is a reference, and nothing it cannot vouch for travels with it
  {
  ...
  // a session control that is on says what it does
  {
  ...
  // a condition that names things names ones Conditional Access has
  {
...
test('a requirement is read exactly: a strength by its own combinations, OR as either branch, a device requirement apart from an app one', () => {
  // a strength is measured against its own allowed combinations, not a tier it resembles
  {
  ...
  // OR means either: a person who can do one branch is not stranded by the other
  {
    const FIDO_ONLY = 'a1000000-0000-4000-8000-00000000f1d0'
  ...
  // a device requirement and an app requirement are two different requirements
  {
    const snapshot = scan({
...
test('what the scan cannot prove is held unknown, never satisfied and never read as absent: places, token protection, unread controls, contexts, workload risk', () => {
  // a place is where the records show somebody signing in, never a list of countries beside the policy or an attribute on the account
  {
  ...
  // token protection is a requirement the scan cannot prove, and it says so
  {
  ...
  // a control IAMAI has no reading for is submitted and never interpreted
  {
    const snapshot = scan({ registrationDetails: [registered('u1', ['fido2SecurityKey'])] })
  ...
  // a requirement nothing in the scan can prove is unknown, not satisfied
  {
    const snapshot = scan({
  ...
  // a condition the request may carry is decoded or held, never read as though it were not there
  {
    const snapshot = scan({
      registrationDetails: [registered('u1', ['microsoftAuthenticatorPush'])],
  ...
  // a session control the request may carry says what it does, or the policy is held unknown
  {
...
// ---- C. applicability, per operation, and what the schedule does with it ----

test('who an operation reaches is read from the operation, not the step: roles, exclusions, groups where knowable, and the circumstances the records answer', () => {
  // who an operation reaches is read from the operation, not from the step it sits in
  {
  ...
  // the circumstances the records answer are answered, and the rest are held
  {
  ...
  // the operator is in scope of an operation, not of the step
  {
    const ADMIN_ROLE = '62e90394-69f5-4237-9190-012177145e10'
    const snapshot = scan({
...
test('the schedule places every step by what its operations do, and never takes a zero, a short watch or a lockout number from what it cannot read', () => {
  // the schedule places work by what each operation does, and stays conservative where it cannot tell
  {
  ...
  // a step whose policy reaches fewer people than the step does is scheduled on the policy
  {
    const ADMIN_ROLE = '62e90394-69f5-4237-9190-012177145e10'
    const ids = Array.from({ length: 20 }, (_, i) => `u${i}`)
  ...
  // an open policy with nothing to read falls back on nothing at all
  {
    const snapshot = scan({ registrationDetails: [registered('u1', ['fido2SecurityKey'])] })
  ...
  // a held circumstance is never a zero, never a short watch, and never a lockout number
  {
    const ids = Array.from({ length: 20 }, (_, i) => `u${i}`)
    const snapshot = scan({
  ...
  // session-only work is placed, and never on optimistic silence
  {
    const ids = Array.from({ length: 20 }, (_, i) => `u${i}`)
  ...
  // a policy that reaches more people than the step lists is never zero, and takes the full watch
  {
