...
export function buildIcs(steps: Step[], tenantName: string, planId: string, watchThresholdPercent = 5, view?: StepView): string {
  ...
  for (const s of steps) {
    ...
    const v = view ? view(s) : null
    lines.push(fold(`SUMMARY:${escape(v ? v.title : s.title)}`))
    // The calendar entry is the runbook: with a view, what the step says on
    // screen — its why, its dates, its portal path, its done-when lines and what
    // to do if it goes wrong (prompt 53 queue item 7); without one, the engine's
    // own lines (the tests that read them).
    const description = (v
