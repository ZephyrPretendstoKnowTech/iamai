...
export type ScoreExtras = {
  /** Population and readiness the caller already computed for this step. */
  precomputed?: { popIds: string[]; activeIn: number; tenantActive: number; readiness: Readiness }
...
export function scoreResult(result: GoalResult, snapshot: TenantSnapshot, viability: MfaViability[], extras: ScoreExtras = {}): GoalScore | null {
  ...
  // The generator already holds the population and readiness; recomputing them per step is what made 25,000 users slow.
  const pre = extras.precomputed
  const popIds = pre ? pre.popIds : impl && impl.expectedWho.kind !== 'workload' ? [...resolvePopulation(impl.expectedWho, snapshot).ids] : []
  const pop = new Set(popIds)
  const activeIn = pre ? pre.activeIn : viability.filter((v) => pop.has(v.userId) && v.activity === 'active').length
  const tenantActive = pre ? pre.tenantActive : viability.filter((v) => v.activity === 'active').length
  const readiness = pre ? pre.readiness : readinessFor(goal.id, popIds, viability, snapshot)
