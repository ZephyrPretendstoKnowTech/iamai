...
test('demo first visit: neither group is settled, and no policy step is Ready', () => {
...
test('demo first visit: a gated policy step is held, undated, and names what it waits on', () => {
  const r = runFixture(fixture('demo'))
...
test('the gate leaves the foundation itself, and everything that is not a policy step, alone', () => {
  const r = runFixture(fixture('demo'))
