...
test('a convention is read only above the agreement floor, series and segments included; below it the proposal is the documented pattern, labelled', () => {
  // a serial prefix is read as a series, not as a literal
  {
  ...
  // a four-segment convention keeps all four segments
  {
  ...
  // no detectable convention returns a labelled proposal, never a guess
  {
  ...
  // one policy, or none, is not a convention
  {
  ...
  // agreement is the weakest link, not the strongest
  {
...
test('a proposal continues the series at the tenant\'s width, in its separator and casing, without destroying acronyms', () => {
  // a proposal continues the series at the tenant’s own width
  {
    const c = detectConvention(GOLDEN)
  ...
  // the separator the tenant uses is the separator proposals use
  {
  ...
  // casing is read, and applied without destroying acronyms
  {
