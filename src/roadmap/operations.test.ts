...
test("the channels serialise the operations: a stale action.json reaches none of them, and two operations are two bodies in the step's order", () => {
  // a stale action.json cannot make the JSON or the download disagree with the operations
  {
  ...
  // two operations are two bodies, in the step’s order, on every channel
  {
...
test('an update validates what it submits and reads the tenant policy it does not; with no complete target, or one its body contradicts, the step is unavailable', () => {
  // an update with no complete target, or a target its body contradicts, makes the whole step unavailable
  {
  ...
  // an update validates what it submits, and reads the tenant policy it does not
  {
    const target = {
      id: 'p-1',
...
test('a stale unavailable step goes through the schedule and comes out of every structure', () => {
  ...
  const ics = buildIcs([stale], 'Contoso', 'plan-x', () => ({ title: 'Stale', why: '', state: '', lane: '', substatus: null, reason: null, fact: null, next: null, who: null, population: null, whatToDo: [], fix: [], beforeTurnOn: [], doneWhen: [], ifWrong: null, dates: null, implementation: false, operation: null, undated: false }))
...
test('a complete policy is a name, a state, a scope and a real control, and a create is offered only for the shapes IAMAI writes', () => {
  // a complete policy is a name, a state, a scope and a real control
  {
  ...
  // a create is offered only for the shapes IAMAI writes, and an unknown one is never quietly dropped
  {
...
test('the strand verdict, the dependencies, the notice, the observation, denial, prompting and batching all follow the policy, not the family', () => {
  // the strand verdict, the dependencies, the notice and the observation all follow the policy, not the family
  {
    const snapshot = {
  ...
  // deny, prompt, strand and batching follow the policy, not the goal it is filed under
  {
...
test('every requirement is judged on its own, AND and OR are not the same policy, and a policy IAMAI cannot read in full never reads as safe', () => {
  // every requirement is judged on its own, and AND and OR are not the same policy
  {
  ...
  // a policy IAMAI cannot read in full never reads as safe
  {
    for (const grant of [
