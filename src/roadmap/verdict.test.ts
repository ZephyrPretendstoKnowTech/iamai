...
// Authored steps: only the fields the verdict reads, cast once at the boundary
// rather than at every call site.
function step(over: Record<string, unknown> & { id: string }): Step {
  const ids = ((over.population as { ids?: string[] } | undefined)?.ids) ?? ['u0', 'u1']
  return {
    goalId: over.id,
...
test('three days and two sign-ins is Not enough evidence, and never reads as ready (item 8)', () => {
  const s = step({ id: 'b', tracking: { daysInReportOnly: 3, signIns: 2, failures: 0, interruptions: 0, failuresByUser: [], evidenceQuality: 'thin' } })
...
test('a quiet person never blocks the verdict; they are named in it', () => {
  ...
  const s = step({ id: 'c', tracking: { daysInReportOnly: 7, signIns: 20, failures: 0, interruptions: 0, failuresByUser: [], evidenceQuality: 'enough' } })
...
test('the recency boundary is exactly RECENT_WINDOW_DAYS', () => {
  const s = step({ id: 'e', population: { total: 2, active: 2, admins: 0, guests: 0, ids: ['inside', 'outside'] } })
...
test('a step that is not in report-only has no verdict', () => {
  assert.equal(verdictFor(step({ id: 'j', status: 'done' }), snapshot({}), NOW, null), null)
  assert.equal(verdictFor(step({ id: 'k', status: 'blocked' }), snapshot({}), NOW, null), null)
})
