...
test('1-4, 18. a stored confirmation is kept whatever a scan sees, and is actionable only while this scan reads its group present', () => {
  // 1. confirmed and present: the operator's answer stands and is the id anything downstream may use
  {
  ...
  // 2. confirmed and unverifiable: the answer is kept, nothing may use it, and no recommendation replaces it
  {
  ...
  // 3. unknown then present: the same stored answer becomes usable again with no new decision
  {
  ...
  // 4. confirmed and proved gone: invalidated, unresolved, and not replaced
  {
  ...
  // 18. a previous scan's reading is not this scan's: present does not survive into a scan that cannot verify
  {
    const snapshot = snapshotOf(policiesExcluding([X]))
    const mapping = mappingOf(X)
...
// ---- 5-12, U1-U3: what the directory read proves, and what detection may conclude ----
//
...
test('5, 8-12, U1-U3. detection: a candidate needs its group read, none-found needs complete evidence, and a recommendation is never a confirmation', () => {
  // 5. a policy naming a group is not the group existing
  {
  ...
  // 8. a group that exists whose members would not enumerate is present with no member count invented
  {
  ...
  // 9. nothing found on evidence IAMAI could not complete: no none-found, no create, no recommendation
  {
  ...
  // 10. nothing found on complete evidence: none-found is allowed, and no id is fabricated
  {
  ...
  // 11. one verified candidate on complete evidence: recommended, and nothing more
  {
  ...
  // 12. two verified candidates: ambiguous, and IAMAI picks neither
  {
    const c = exclusionsGroupChoice({
      snapshot: snapshotOf(policiesExcluding([X, Y])),
  ...
  // U1. every policy-referenced group read, nothing qualifies, universe partial: undetermined, not none-found
  {
  ...
  // U2. a suitable group outside the read universe: IAMAI does not conclude there is none
  {
  ...
  // U3. reading one more group does not complete a partial universe
  {
...
test("6-7. presence: only Graph's own not-found is absence; every other failure is unknown", async () => {
  // 6. a generic Graph failure is unknown, never absent
  {
  ...
  // 7. Graph's own not-found, end to end through the request layer, is absence
  {
...
test('13, 14, 19. a real plan: a stored group this scan cannot verify, or Graph proves gone, reaches no policy, and nothing fills its place', () => {
  // 13. a real plan: a stored confirmation this scan cannot verify reaches no policy, and is not called deleted
  {
  ...
  // 14. a real plan: a confirmed group Graph proves gone is distinguished from one it could not check
  {
    const f = fixture('small')
    const stored = operatorExclusionsDecision(f.mapping)!.id
    const partial = new Map(f.groups)
  ...
  // 19. Foundation A: an unresolved safety input leaves the operation unresolved, with no fallback filling it
  {
    const f = fixture('small')
    const stored = operatorExclusionsDecision(f.mapping)!.id
    const partial = new Map(f.groups)
...
test("16. detection writes nothing, and only an operator's confirmation ticks the exclusions group, on any fixture", () => {
  // 16. detection writes nothing: scan, detect, recommend, plan, scan again, and the record does not move
  {
  ...
  // no picker default and no detected pass can tick the exclusions group, on any fixture
  {
    for (const f of allFixtures()) {
...
test("L1-L3. a legacy detected record is read and shown, never the operator's answer, and no wizard pass, scan or load turns it into one", () => {
  // L1. a legacy detected record, its group present: read it, show it, and never call it the operator’s answer
  {
  ...
  // L2. the wizard’s own detected answer does not make coverage call the exclusions confirmed
  {
  ...
  // L3. a scan does not migrate a detected record into a confirmation
  {
    const snapshot = fixtureSnapshot()
...
test('G1-G3. a blocking exclusion-group check that has not passed holds the step, the gate and the policies', () => {
  ...
  assert.equal(goodStep.status, 'done', 'the valid group completes without the removed pre-change recovery proof')
...
test('G4. a warning does not hold the step, and a failing warning does not hold a policy', () => {
  ...
  const step = r.steps.find((s) => s.id === EXCLUSIONS_STEP)!
  assert.equal(step.status, 'done', 'the warning does not block the step')
  assert.ok(step.checks?.items.some(item => item.fix === 'not-mail-enabled'), 'the warning remains visible after the blocking checks pass')
})
