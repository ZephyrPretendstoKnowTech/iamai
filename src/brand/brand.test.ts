...
import { MARK_COLORS, MASTER, RASTER, derived, geometry, normalise } from '../../scripts/brandDerive.ts'
...
test('brand: the master mark is the approved bytes, pure themeable vector geometry with its topology, and no text', () => {
  assert.ok(existsSync(MASTER), `${MASTER} is missing: the brand has no master`)
  assert.equal(manifest().logo.master, MASTER)
  const svg = master()
  assert.match(svg, /viewBox="0 0 100 100"/, 'the master is drawn on a 0 0 100 100 grid')
  ...
  assert.equal(svg, read(APPROVED_MASTER), `${MASTER} is not byte-for-byte ${APPROVED_MASTER}`)
  ...
  // Nothing but vector geometry, and nothing outside itself.
  for (const [what, re] of [
    ...
    ['an external reference', /\shref=|xlink:href|url\(/i],
    ['text', /<text\b|<tspan\b/i],
  ] as const) {
    assert.doesNotMatch(svg, re, `the master must not contain ${what}`)
  }
  assert.deepEqual([...svg.matchAll(/https?:\/\/[^\s"']+/g)].map((m) => m[0]), ['http://www.w3.org/2000/svg'], 'the only URL in the master is its namespace')
  // <title> and <desc> name the file; scripts/brandDerive.ts strips them from the geometry the shell draws.
  assert.doesNotMatch(geometry(svg), /<title\b|<desc\b/i, 'the drawn geometry carries no metadata element')
  assert.equal(svg.replace(/<(title|desc)\b[\s\S]*?<\/\1>/g, '').replace(/<[^>]*>/g, '').trim(), '', 'the master renders no text of its own')
  // One colour source, and it is themeable.
  assert.match(svg, /currentColor/, 'the master inherits its colour')
  assert.deepEqual([...svg.matchAll(/#[0-9a-f]{3,8}\b/gi)].map((m) => m[0]), [], 'the master hard-codes no colour; the derivatives carry the values')
  // Two flat filled panels, the recorded paths in order; the right panel's fifth point is the lintel.
  ...
  const { logo } = manifest()
  assert.deepEqual([logo.family, logo.variant, logo.version], ['Threshold', 1, 2])
  const g = logo.geometry
  assert.deepEqual([...svg.matchAll(/ d="([^"]+)"/g)].map((m) => m[1]), [g.leftPanelPath, g.rightPanelPath], 'the master draws the recorded paths, in order')
...
test('brand: every derived asset is exactly what the master derives to, in the owner’s mark colours', () => {
...
test('brand: the served pages ask for the published brand icon, and its raster is a 32x32 PNG', () => {
...
test('brand: there is no tagline, in the manifest or in any logo asset’s name or bytes', () => {
  const m = manifest()
...
test('brand: the manifest carries the owner palette exactly, production paints it, and brand teal is not success green', () => {
  ...
  for (const [theme, wanted] of [['light', LIGHT], ['dark', DARK]] as const) {
    ...
    assert.deepEqual(Object.keys(palette[theme]).filter((k) => k !== 'name').sort(), Object.keys(wanted).sort(), `${theme} has a role the owner did not approve`)
  ...
  // Task 029 recorded the palette and task 030 applied it: if src/ui/tokens.ts and
  // the manifest can disagree, the interface is wearing a colour nobody approved.
  for (const [theme, tokens] of [['light', TOKENS_LIGHT], ['dark', TOKENS_DARK]] as const) {
    for (const role of BRAND_ROLES) {
      assert.equal((tokens as Record<string, string>)[role].toUpperCase(), palette[theme][role].toUpperCase(), `${theme}.${role}: src/ui/tokens.ts and ${MANIFEST} disagree`)
    }
  }
...
test('brand: the interface is set in the three approved families, from faces staged here and recorded byte for byte', () => {
  ...
  assert.equal(typography.appliedToProduction, true)
  assert.deepEqual(typography.families, { display: 'IBM Plex Serif', interface: 'IBM Plex Sans', technical: 'IBM Plex Mono' })
  ...
  // The provenance record exists so a reader can re-verify a face against IBM's
  // published package. A hash that drifted from the file it names proves nothing.
  const provenance = read(PROVENANCE)
  assert.ok(provenance.includes('SIL Open Font License'), 'the provenance record names the licence')
  const rows = [...provenance.matchAll(/\| `(IBMPlex[A-Za-z0-9-]+\.woff2)` \|[^|]*\|[^|]*\| `([0-9a-f]{64})` \|/g)]
  const staged = readdirSync('public/fonts').filter((f) => f.endsWith('.woff2'))
  assert.deepEqual(rows.map((m) => m[1]).sort(), [...staged].sort(), 'the provenance table and public/fonts name different faces')
...
test('brand: the four approved application authorities are named, hashed and untouched, and every file the manifest names exists', () => {
