...
// A decision's tileNoteUnread is its tile note where IAMAI could not read the
// sign-in records (stepContract.ts unsavedTiles), and the review page draws no tiles.
// A step's whatToDo.verificationLead opens its workflow checks on the saved
// answer (stepResources.ts verificationResourceLines), and the review page draws
// no checks and saves no answer.
// shared.policyDoneWhenUnobserved is an enforced policy's Done-when where IAMAI watched
// no report-only period for it (doneWhen.ts), and the review page draws no enforced step.
