...
test('session-lifetime report-only and ready to enforce read the browser policy alone, by its own id: verified, then turned on, with no Policy B and Enforce withheld', () => {
  // session-lifetime report-only: the browser policy alone, verified by its own id, with no unmanaged id asked for
  {
  ...
  // session-lifetime ready to enforce: the browser policy is turned on by its own id; nothing names a Policy B to turn on; Enforce stays withheld
  {
