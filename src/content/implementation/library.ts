...
export type LibraryPackage = { dir: string; stepId: string; pkg: CompiledPackage; withheld: string[] }
...
export function compileLibrary(root: string = LIBRARY_ROOT): { registered: LibraryPackage[]; notSteps: LibraryPackage[] } {
  ...
  for (const dir of packageDirs(root)) {
    ...
    const entry = { dir: dir.replaceAll('\\', '/'), stepId: pkg.meta.stepId, pkg, withheld }
    const content = contentStepForPackage(entry.stepId)
    if (!content) {
      notSteps.push(entry)
      continue
    }
...
export function registryOf(library: { registered: LibraryPackage[] }): { $comment: string; packages: Record<string, CompiledPackage> } {
  return {
