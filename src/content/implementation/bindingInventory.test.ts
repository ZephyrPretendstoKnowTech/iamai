...
test('no binding carries a source reference still waiting on an answer, and a step waiting on references binds none of the fields they would complete, still binding its name and its grant', () => {
  // no binding carries a source reference still waiting on an answer
  {
    let checked = 0
...
test('a hold does not erase a target IAMAI knows, and a whole policy that sets no grant or no session binds that root as null', () => {
  // a hold does not erase a target IAMAI knows: a step held on references binds every target field no reference touches
  {
    let found = 0
    for (const { name, r, ctx } of plans()) {
      for (const step of r.steps) {
  ...
  // null is a value: a whole policy that sets no grant or no session binds that root as null
  {
    let found = 0
    for (const { name, r, ctx } of plans()) {
      for (const step of r.steps) {
