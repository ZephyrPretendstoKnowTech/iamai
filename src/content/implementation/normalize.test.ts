...
test('normalisation keeps the author’s words: an Email’s audience and trigger in any shape (never guessed), content under another name for a state, and a script mode IAMAI cannot call withheld with its reason', () => {
  // an Email declares its audience and trigger in any of the library’s shapes, and a one-state Email is sent for that state
  {
    const meta = {
      stepId: 'x',
      requiredBindings: [],
  ...
  // content authored under another name for a runtime state is that state’s, and never replaces what the author wrote for the state itself
  {
  ...
  // a script mode IAMAI cannot call is withheld with its reason, and the modes it can call still project
  {
...
test('troubleshooting written as sentences and readiness evidence written as a paragraph reach the runtime’s shape; a tile stating a tenant fact stays refused', () => {
  // a troubleshooting scenario written as sentences becomes the runtime’s lists, in its block’s states, titled by its symptom
  {
  ...
  // readiness evidence written as a paragraph, safe-now and safe-to-enforce lines reaches the evidence view; a tile stating a tenant fact stays refused
  {
