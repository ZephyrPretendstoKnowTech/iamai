...
test('every state a registered package authors is a runtime state or reconciled with one, none leaves a reachable action unsafe, and an unsafe or undefined state is reported', () => {
  // every state a registered package authors is a runtime state or reconciled with one, and none leaves a reachable action unsafe
  {
  ...
  // a state gated on an object is reported unsafe where a reachable action deploys without that object
  {
    const pkg = {
