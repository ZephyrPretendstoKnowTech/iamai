...
test('s-shared-devices: Create is one call after the whole script, with the name, the accounts, the exclusions and the trusted location', () => {
...
test('s-shared-devices: with no single trusted location the script is withheld, never drawn as a call that would throw', () => {
...
test('s-shared-devices: a conditions correction calls CorrectConditions on the policy with the resolved values, and moves nothing to report-only', () => {
...
test('s-shared-devices: Verify is called on the policy, Enforce is withheld with its reason', () => {
