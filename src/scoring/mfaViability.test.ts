// §10.7 test cases T1–T14 (docs/design/collection.md). now = 2026-08-26,
// evidence covered = last 30 days unless stated.
import { test } from 'node:test'
import assert from 'node:assert/strict'
import { scoreMfaViability, summarizeTenant, sortViability } from './mfaViability.ts'
...
test('T1: stale Authenticator but observable in window, never challenged → notChallenged', () => {
  ...
  assert.equal(r.state, 'notChallenged')
...
test('T3: Authenticator registered 6 days ago, no evidence → likelyViable via recentRegistration', () => {
  ...
  assert.equal(r.state, 'likelyViable')
...
test('T9: 200 days since sign-in, Authenticator up to date → inactive wins over likelyViable', () => {
  const r = scoreMfaViability(
    input({
...
test('T14: admin unverified sorts first in tenant-level ordering', () => {
  const base = input()
  ...
  assert.equal(admin.state, 'unverified')
  const sorted = sortViability([member, admin])
  assert.equal(sorted[0].userId, 'admin-user')
  const summary = summarizeTenant([member, admin])
