...
test('the page and the campaign share one readiness; Plan gates use their actual target method cohort', () => {
  for (const name of PLANS) {
    ...
    // The counts are the rows, and they partition the active people.
    ...
    assert.equal(READINESS_STATES.reduce((n, s) => n + view.counts[s], 0), view.facts.active, `${where}: the states sum to the active people`)
