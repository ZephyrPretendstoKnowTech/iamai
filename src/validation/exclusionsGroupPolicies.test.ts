...
// need the emergency exclusions group. Every policy that is On or Report-only
// needs it, whatever it reaches today (owner, that afternoon: "all policies —
// that's the intent from Jon's baseline"); Step 2's completion, its Policy
// exclusions tile and Step 4 all read the same answer.
...
test('a policy that reaches no emergency account today still needs the group, in all three', () => {
  ...
  assert.notEqual(step2.status, 'done')
  assert.equal(tile.items!.some(item => item.subjectId === 'ro-other'), true)
  assert.ok(Object.values(step4).some(state => state !== 'ready'))
})

test('unread excluded groups are unknown, never a confirmed missing exclusion; an excluded group is a pass whatever the reach', () => {
  const f = tenantWith(null)
  ...
  assert.equal(exclusionsGroupPolicies(input(reportOnly('unread-reach', { includeGroups: ['group-never-read'] })))[0].outcome, 'fail', 'an unread reach is no excuse: the group belongs on every policy')
  const unreadExclusions = reportOnly('unread-exclusions', { includeUsers: ['All'] }) as { conditions: { users: Record<string, unknown> } }
  delete unreadExclusions.conditions.users.excludeGroups
  assert.equal(exclusionsGroupPolicies(input(unreadExclusions))[0].outcome, 'unknown')
