...
const METHODS = { url: 'https://learn.microsoft.com/entra/identity/authentication/overview-authentication', label: 'Microsoft: authentication methods' }
...
export const RULE_TEXT: Record<string, { what: string; why: string; label?: string }> = {
  ...
  'bg.perUserMfaOff': {
    ...
    // is read and when the step is on the plan are two sentences.
...
export const FINDING = {
  ...
  bgNoRecordedDrill: 'no recovery test recorded for this account; a successful sign-in alone does not confirm recovery was tested',
  bgNeverSignedIn: 'has never signed in: nobody knows whether the credential works',
  bgDrilled: (date: string, days: number): string => `recovery test recorded ${date}, within the last ${days} days`,
