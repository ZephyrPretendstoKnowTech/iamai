...
export type UserRow = {
  ...
  /** True when the directory read selected signInActivity and succeeded. Graph omits the property for an account that never signed in, so on that read its absence means "never signed in" (collectors.ts mapUser). */
