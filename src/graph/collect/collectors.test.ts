...
test('the dedicated Fido2 read replaces the partial parent method with its provenance, a refused one keeps the parent data and the precise failure, and neither manufactures a methods collection', async () => {
  // dedicated v1.0 Fido2 read replaces the partial parent method and records its provenance
  {
  ...
  // a refused Fido2 relationship read preserves the parent data and precise failure
  {
  ...
  // a dedicated Fido2 response does not manufacture an otherwise missing methods collection
  {
    const section = await withFetch({
...
test('the migration state comes from v1.0, else from beta with a note saying so, else the section says it is absent, and a failed beta read leaves the v1.0 section standing', async () => {
  // v1.0 carries the field: no beta read, no fallback note
  {
    let betaCalls = 0
    const section = await withFetch(
      {
  ...
  // v1.0 lacks the field: beta supplies it and the section says where it came from
  {
    const section = await withFetch(
      {
  ...
  // neither read carries the field: the section is still ok, and says the field is absent
  {
    const section = await withFetch(
      {
  ...
  // a failed beta read is tolerated: the v1.0 section stands
  {
    const section = await withFetch(
      {
...
test('sign-in activity: a failed attempt is never the last successful sign-in, a missing signInActivity on a read that selected it means none on record, and a read that could not select it marks nobody read', async () => {
  // a failed sign-in attempt never becomes the last successful sign-in
  {
  ...
  // a successful read marks sign-in activity read for every account, including one Graph returns without signInActivity
  {
  ...
  // each page handed on while reading carries the same reading of sign-in activity as the result
  {
    const pages: boolean[][] = []
    await withFetch({
  ...
  // a read that could not select signInActivity marks nobody read
  {
...
test('the method inventory: passkey last use only where beta reports it, Platform SSO as its own method, dedicated FIDO fields merged with their endpoint, and a failed FIDO detail read keeps the generic inventory', async () => {
  // item 2: a beta row that reports lastUsedDateTime as null marks the passkey never used; one without the field leaves it unknown
  {
    const original = globalThis.fetch
    globalThis.fetch = (async (input: RequestInfo | URL) => {
      const url = String(input)
  ...
  // item 11: a Mac’s Platform SSO credential is read as its own method, never "other"
  {
    const original = globalThis.fetch
    globalThis.fetch = (async (input: RequestInfo | URL) => {
      const url = String(input)
  ...
  // dedicated FIDO reads merge exact credential fields and endpoint provenance
  {
    const original = globalThis.fetch
    let v1Batch = 0
    globalThis.fetch = (async (input: RequestInfo | URL) => {
  ...
  // failed FIDO detail batches preserve the generic method inventory
  {
    const original = globalThis.fetch
    let calls = 0
    globalThis.fetch = (async (input: RequestInfo | URL) => {
...
test('cross-tenant access keeps defaults and partner overrides apart, an unavailable relationship is partial, and the collector requests exactly its registry row\'s paths', async () => {
  // cross-tenant collection keeps defaults and partner overrides separate
  {
    const section = await withFetch({
  ...
  // an unavailable cross-tenant relationship is partial rather than no trust
  {
    const section = await withFetch({
  ...
  // the cross-tenant collector requests its registry row's path and the paths the row says it also reads, and nothing else
  {
...
test('a method read that failed for a passing reason is read again, once and bounded, after the batch Retry-After; a refused one is not; a failed batch is read person by person (owner item 4, 2026-09-19)', async () => {
  // a throttled or failed method read is read again after the batch Retry-After; a refused one is not
  {
    const waits: number[] = []
  ...
  // a method batch that fails outright is read person by person rather than losing the page
  {
  ...
  // re-reads are bounded: a person still failing stays unknown, and a second failure leaves the rest of the batch for the next scan
  {
    const f = methodsFetch(
...
test('a person whose method read still failed gets their own registration report row, and a refused fallback leaves them unknown without failing the scan', async () => {
  // a person whose method read still failed gets their own row of the registration report
  {
  ...
  // a refused registration fallback leaves the people unknown and never fails the scan
  {
    const original = globalThis.fetch
