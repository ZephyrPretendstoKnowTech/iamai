...
export const COLLECTOR_REGISTRY: CollectorSpec[] = [
  ...
  { name: 'Role assignments', lane: '0', configKey: 'roleAssignments', endpoint: '/roleManagement/directory/roleAssignments', version: 'v1.0', paged: true, scopes: ['RoleManagement.Read.Directory'], requiredCapability: null, gate: 'none', purpose: 'Active admin roles per user for admin-targeting intents.' },
  ...
  { name: 'Signed-in operator', lane: '0', configKey: 'me', endpoint: '/me', version: 'v1.0', scopes: ['User.Read'], requiredCapability: null, gate: 'none', purpose: 'Operator identity recorded in the plan file.' },
  { name: 'Operator groups', lane: '0', configKey: 'meMemberOf', endpoint: '/me/memberOf', version: 'v1.0', paged: true, scopes: ['User.Read'], requiredCapability: null, gate: 'none', purpose: 'Warn when the operator sits inside groups a plan step targets.' },
  // ---- Lane A: aggregates ----
  ...
  { name: 'Name resolution', lane: 'on-demand', endpoint: '/directoryObjects/getByIds', version: 'v1.0', scopes: ['Directory.Read.All'], requiredCapability: null, gate: 'runs only for ids the UI would otherwise show raw', purpose: 'Show display names instead of GUIDs, everywhere.' },
]
