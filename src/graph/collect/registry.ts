...
export const COLLECTOR_REGISTRY: CollectorSpec[] = [
  ...
  { name: 'Group transitive members', lane: 'on-demand', endpoint: '/groups/{id} ($select=id,displayName,membershipRule,mailEnabled,securityEnabled,groupTypes,isAssignableToRole) + /groups/{id}/transitiveMembers (+ $count)', version: 'v1.0', paged: true, scopes: ['Directory.Read.All'], requiredCapability: null, gate: 'runs only for groups named by scanned policies, authentication-method profiles, role assignments, or operator choices; count-and-sample above 20k', purpose: 'Group type, assignment suitability, name, dynamic rule, affected-population counts and exclusion-group sanity checks.' },
