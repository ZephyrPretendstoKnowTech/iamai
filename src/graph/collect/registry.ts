...
export const COLLECTOR_REGISTRY: CollectorSpec[] = [
  ...
  { name: 'Signed-in operator', lane: '0', configKey: 'me', endpoint: '/me', version: 'v1.0', scopes: ['Directory.Read.All'], requiredCapability: null, gate: 'none', purpose: "The operator's directory id (derive/operator.ts), for bg.notPersonal, the operator-passkey step and the steps' \"your own account\" lines. The plan file records the signed-in MSAL account, not this read." },
  { name: 'Operator groups', lane: '0', configKey: 'meMemberOf', endpoint: '/me/memberOf', version: 'v1.0', paged: true, scopes: ['Directory.Read.All'], requiredCapability: null, gate: 'none', purpose: 'The groups the operator is in. Nothing reads config.meMemberOf yet; whether to keep the read is an owner question.' },
  // ---- Lane A: aggregates ----
  ...
  { name: 'Auth methods', lane: 'A', sourceKey: 'authMethods', endpoint: '/users/{id}/authentication/methods and /users/{id}/authentication/fido2Methods ($batch of 20), then beta /users/{id}/authentication/fido2Methods ($batch of 20) for accounts holding a passkey', version: 'v1.0', scopes: ['UserAuthenticationMethod.Read.All'], requiredCapability: null, gate: 'a throttled or failed generic-method read is read again for that user alone, after the batch Retry-After; a user still unread is unknown and the registration report stands in; a failed FIDO2 detail read leaves credential fields unresolved', purpose: 'Registered method inventory plus exact passkey model, AAGUID, type and attestation detail (values stripped; never phone numbers).' },
  ...
  // Read with the sign-in records: laneB.ts takes its path from this row, so the
  // page cannot list a different read from the one the scan makes. Every
  // category, because a change to an emergency account, its exclusions group, a
  // Conditional Access policy or the passkey policy each dates a recovery test
  // (roadmap/cleanupDone.ts); nothing else is kept from an event.
  { name: 'Directory audit events', lane: 'B', endpoint: '/auditLogs/directoryAudits', version: 'beta', paged: true, scopes: ['AuditLog.Read.All'], requiredCapability: 'entraP1', gate: 'Entra ID P1/P2; read with the sign-in records, the last 30 days of every category', purpose: "Each event's activity, category, result and the ids of the objects it changed, to date changes to the emergency accounts, their exclusions group, Conditional Access policies and the passkey policy." },
  // ---- On demand ----
