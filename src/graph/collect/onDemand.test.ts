...
test('group reads fail closed: unreadable licence evidence stays unknown, and a member row without an id is refused', () => {
  // saved-group license evidence stays unknown when the collection or an identifier is unreadable
  {
  ...
  // direct membership rejects a row without a stable identifier instead of certifying an empty result
  {
