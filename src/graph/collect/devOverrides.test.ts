...
test('the scan overrides in the query string are honoured only by a dev build under ?dev=1, and the scan reads them through that gate', () => {
  // a published build ignores the scan overrides in the query string
  {
    assert.deepEqual(devScanOverrides('?dev=1&licence=free&fail=1', false), { devFail: false })
  }
  // a dev build honours them only under ?dev=1
  {
  ...
  // the scan reads its overrides through the dev-build gate
  {
