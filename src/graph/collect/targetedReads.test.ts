...
test('only people with a phishing-resistant method last seen in the unread part are read, and only over that part', () => {
  // only people with a phishing-resistant method whose last sign-in falls in the unread part are read
  {
  ...
  // a person’s read asks for their interactive sign-ins in the unread part only
  {
...
test('a targeted read joins the person\'s evidence, and turns Unknown into a judgement on their sign-ins', () => {
  // the rows read join the person’s evidence, and the person reads as covered
  {
  ...
  // a partial read without the targeted read is Unknown; with it, the person is judged on their sign-ins
  {
...
test('a person\'s read honours Retry-After past the default ceiling, stops at its own 5xx bound, follows nextLink, and drops what the bulk read folded', async () => {
  // a throttled sign-in read keeps honouring Retry-After past the default ceiling, then reads
  {
  ...
  // a sign-in read retries server errors past the default ceiling, and stops at its own
  {
  ...
  // a person’s read follows nextLink to the start of the window, so a busy person is read whole
  {
  ...
  // a person’s records at or after where the bulk read began are dropped: that read folded them already
  {
