...
**13. Baseline/source safety conflict.** §9.4 gives a restrictive control whose canonical scope reaches the emergency identities one of two outcomes. `s-goal-require-managed-device` is outcome (1): the pinned object (660ab461) excludes the exclusions group, so §10.3 carries `create ← s-prereq-exclusion-group@complete` and records no `baselineSafetyConflict`; with the group Completed, the device decision made and the trusted location Completed, not started → **Ready · Create**, then **Ready · Observing** once it exists in Report-only. Outcome (2) — `baselineSafetyConflict` recorded on `enforce`, so the step reaches **Ready · Create** and **Ready · Observing** but goes **On Hold · Baseline target is unsafe as authored** the moment the evidence predicate is satisfied and its next action becomes `enforce` — is `s-goal-unmanaged-browser` (§10.3 note), which the pinned baseline does not hold (§10.0 `generated: no`), so `dependency-data.json` carries no such edge today. IAMAI never patches the exclusion in on its own.
...
- **V3** — cleared, permitted. Neither page (Appendix C) contains a sentence restricting the creation of a Report-only policy while Security Defaults is enabled; the Security Defaults page requires disabling it to implement replacement policies. Gate stays on `enforce`. Re-checked 2026-09-20 against the V1 audit (S4-16 / S4-20): the report-only page (updated 2026-06-01) still contains no sentence about Security Defaults, and https://learn.microsoft.com/entra/fundamentals/security-defaults (updated 2026-07-01) still reads "Organizations that choose to implement Conditional Access policies that replace security defaults must disable security defaults" and "After administrators disable security defaults, organizations should immediately enable Conditional Access policies". The edges are unchanged; what was wrong was the step's own words, which named only the direction that blocks nobody — corrected in `s-prereq-security-defaults` `whatToDo.lead`.
- **V3 corroborated, 2026-09-21** (owner asked for research beyond Learn, doubting that a policy can be created at all while Security Defaults is on). Three sources, none of which is Learn's silence: (a) Microsoft Q&A, MVP answer on moving from Security Defaults to Conditional Access — "To ensure a transition, create those CA policies first, then turn off Security Defaults and have those CA policies enabled"; (b) practitioner cutover guidance — "You should not disable Security Defaults while replacement policies are still in report-only mode... complete coverage analysis and pilot testing first, then use a coordinated cutover that disables Security Defaults and immediately enables the approved equivalent Conditional Access foundation", which presupposes report-only policies existing beforehand; (c) Microsoft's planning guidance of at least a week of report-only observation per policy, which can only happen before the cutover. No source in any of them describes the portal refusing to create a policy while Security Defaults is enabled. The plan's order stands and the step may now state it.
