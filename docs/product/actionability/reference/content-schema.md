...
| `impact` | `{ fallbackLabel?: string }` | no | yes (`RUNTIME_META_KEYS`) | Validated: `fallbackLabel` is a non-empty string (`impact.fallbackLabel: a non-empty string`). What the Plan row's Impact column reads where the step reaches no people (U13): `ui/surfaces/rowWho.ts` reads the people count, then this label, then the placeholder `—`. Added in B2; authored in B8. |
| `milestone` | `{ actionText?: string }` | no | yes (`RUNTIME_META_KEYS`) | Validated: `actionText` is a non-empty string (`milestone.actionText: a non-empty string`). The words under the opened step's milestone date in its action column (U3): `ui/surfaces/stepBody.ts` hands it to `stepContract.ts railOf`; absent, the date stands alone and no sub-line is generated. Added in B3; authored in B8. |
