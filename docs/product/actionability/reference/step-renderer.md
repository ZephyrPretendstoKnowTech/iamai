...
- **(plan record)**: what the operator saved, loaded from IndexedDB database `iamai`, version 8 (`cache.ts:75`). The `mapping` store holds `MappingState` (`loadMappingState`). The `plan` store holds `PlanDecisions` (`loadPlanRecord`, `cache.ts:202`): skips, `stepDecisions`, `confirmations`, `checkpoints`, `observations`, `signature`, start, freeze and first deployment. Both are loaded in `planData.ts:182-190` and persisted back in `planData.ts:342-370`.
