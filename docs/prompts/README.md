...
| `50-demo.md` | The last prompt of the rebuild: the 49.1 verification findings (screen-only print line, next-working-day default, unconditional passkey clause, row name/gap fitting, waiting vs stop, no build numbers, first-click sign-in, print page 1 as posture summary), then a demo tenant built to show the finished product with a week-two tracking story reached by Re-scan | 49.1 done (962a09a); target-state.md and page-contracts.json updated by the reviewer |
