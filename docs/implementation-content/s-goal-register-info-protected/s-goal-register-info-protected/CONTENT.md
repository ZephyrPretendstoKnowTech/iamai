...
Re-open the policy by the same policy ID. Confirm it is still **Report-only**, its conditions, grant and session controls match the intended target, and the controlled registration test is done.
...
If any one of them is not true, leave the policy in Report-only. Change **Enable policy** to **On** and save. Verify after the change: a test account can register a method through the intended route, the approved recovery process still works, and emergency access still works. Then rescan IAMAI.
@@IAMAI-END
