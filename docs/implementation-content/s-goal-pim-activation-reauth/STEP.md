...
- **Missing:** create and publish the dedicated authentication context where no context has its ID, then create the CA policy in Report-only. IAMAI does not read authentication contexts, so it cannot tell a published context from a missing one, or one this plan prepared from one something else already uses, and never enters **Context missing**: the preparation is the create's first instruction until a scan can read contexts (R4-18). It creates, and never renames or reuses: a context with the ID under any other name or description stops the step, in the Entra procedure and in the script, whose Create also refuses a context this step did not prepare.
