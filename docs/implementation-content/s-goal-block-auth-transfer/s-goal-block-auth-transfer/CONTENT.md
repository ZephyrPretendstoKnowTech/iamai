...
Re-open the policy by the same policy ID. Confirm it is still **Report-only**, its conditions, grant and session controls match the intended target, and the step's evidence has been reviewed.
...
If any one of them is not true, leave the policy in Report-only. Change **Enable policy** to **On** and save. Verify after the change: affected users can sign in directly on the destination device, and emergency access still works. Then rescan IAMAI.
@@IAMAI-END
