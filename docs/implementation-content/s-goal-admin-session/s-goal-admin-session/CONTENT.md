...
Assess Report-only evidence for **Shorten Admin Sessions** in {{tenant.displayName}}: {{evidence.reportOnly}}. Use the exact readiness conditions in STEP.md and do not recommend enforcement merely because no failures appeared.
@@IAMAI-END
...
If prompts arrive unexpectedly, verify which policy applied, the browser session, device account state, and other session-frequency policies. Do not “fix” this by adding exclusions before identifying the actual policy interaction.
@@IAMAI-END
