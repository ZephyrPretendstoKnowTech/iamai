...
| Walk | `TEMP=../cache/tmp node --import ../logs/c1/netblock.mjs scripts/walk.mjs`, from 13:41, during the 0989d8a full suite | **0989d8a** (dist from `build-2`), 13:41:06–13:45:07 | 0 | **completed**: "show-ready on this walk (no P0)", **0 P0, 495 P1, 50 P2**; first load throttled 4673 ms. Report `docs/reports/walk-0989d8a.md`, captures `walk/0989d8a/` (gitignored). Against cycle 7's `walk-4361874.md` (the last completed walk; cycle 8 and review 8 have none) after stripping digits (`sh ../scratch/c8-walk-compare.sh`): only the two build-id lines differ (`walk-report-diff.txt`, 8 diff lines) (`walk-1.txt`) |
