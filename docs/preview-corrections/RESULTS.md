...
| C01 F2, prerequisites met | `node docs/preview-corrections/probes/r1-c01-groups-ready.ts`, `REV=1` (`c01-groups-ready.txt`, `c01-groups-ready-rev.txt`) | 0 | listed: all-users `correct` executable:true, blockers [], JSON/PowerShell update the admins-group policy to `includeUsers:["All"]`; reversed: internal policy |
...
## S5 — repair review findings (2026-09-13 23:02–23:32 MDT)

Start: HEAD 078407b, working tree clean, `git stash list` empty — no preserved unfinished diff. Logs: ../logs/s5/. Synthetic fixtures and local probes only; no Graph, no tenant, no browser, no remote.

### Scope taken
Critical R1-F2; high: the red required check foundationB.test.ts:73 and R1-F4. Not changed: R1-F1 and R1-F3 (high) and R1-F5 (medium), with reasons in BLOCKED S5 23:30.

### Expected behaviour (written before the change)
- F2: the all-users goal's target among several of its own policies does not depend on scan order or names. A group-assigned admins policy is never rewritten to All users because it was listed first. Where nothing tells the candidates apart the step holds and hands over nothing: no update and no duplicate create.
- foundationB: the guard counts every status assignment with its reason. A new HTTP error class carrying a response status has the reason the guard already gives for http.ts.
- F4: the all-users AI Info is true in every state its block renders for (partial includes report-only corrections) and states no number that has no value behind it.

### Fixed (commits)
- ae9d246 — coverage.ts `ownCandidate`, plus `reachesWhole`/`assignedToAll` on each candidate. generate.ts picks within each adjust tier through `ownCandidate`; a tie uses the existing `unmatchedPair` hold with a new `ambiguousTarget` flag (types.ts) and its own copy: `pages.plan.blocked.targetAmbiguous`, `pages.app.plan.targetAmbiguous`, contract `doneTarget`, drawn by stateReason.ts, stepContract.ts and stepExport.ts. tracking.ts fingerprint makes the same pick; a tie takes the classifier's `sufficientId` or nothing (no scan-order fallback). foundationB allowance for http.ts 2 → 3. all-users `ai.correct`: "The policy is already enforced on your tenant." → "The policy already exists on your tenant."; the "33% threshold" sentence → "… helps people register a method. Whether each person has one is shown on MFA Readiness, not by this policy." Registry regenerated with the compiler.
- 03e6340 — full run #1 (6 failures) showed the tie hold firing where the tied candidates already reach the goal's whole population: messy admins report-only pair (snapshot Ready · Observing → On Hold), mid legacy-auth (lifecycle enforced → not-deployed), large managed-device readiness gate (→ unmatched-pair). Holding there removed working plans although correcting any of them changes nobody's scope. Narrowed: only a tie whose furthest reach is part of the population holds; wider ties keep the first listed (S1 gap 2, unchanged). Hold copy avoids "user" (footer.test.ts). No snapshot, lane or readiness test was changed to pass.

### Verified after
- `node docs/preview-corrections/probes/s5-f2-hold.ts` (`f2-hold-2.txt`, at 03e6340), prerequisites met, listed and reversed: all-users `adjust`, reason `unmatched-pair` with `ambiguousTarget`, `executable: false`, no operations, PowerShell carries no policy id, tracking null, hold reason "until one policy stands out as this goal's own". `r1-c01-groups-ready.ts` and `SHAPE=groups|roles|mixed r1-c01-groups.ts` (`f2-ready.txt`, `f2-groups.txt`, at ae9d246): groups → no resolution in either order; roles and mixed unchanged (all-users → internal, admins → admin, session → session in both orders).
- Matrix `s3-matrix.ts curated all` (`matrix-2.txt`, at 03e6340): identical to R1's `logs/r1/matrix.txt` apart from timings (490 renders, 0 differing lines). At ae9d246 it had differed on messy admins (the collateral hold that 03e6340 removed).
- NOT VERIFIED: the held step's rendered contract/Implementation text on a fixture (no fixture produces the hold; the probe's stepContract call lacked its context). Lone group-assigned admins policy: FAILS (BLOCKED S5 23:30, `lone-group-admins.txt`).

### Test expectation changes (explained)
- foundationB.test.ts:80 http.ts `n: 2 → 3`, reason unchanged. The third assignment is `GraphResponseShapeError` (S2 e6a73a3), an HTTP error class carrying the response status. The guard's own instruction is to add the file with its reason.
- mfaAuthContentSpecs.test.ts (all-users AI): pinned the two F4 sentences; now pins the corrected sentences and adds `doesNotMatch(/already enforced|\d+% threshold/)`.
- content.test.ts known non-rendered list: `.pages.plan.blocked.targetAmbiguous` added beside `.pages.plan.blocked.pairUnmatched` (a blocked-row reason read through reasons.ts, not the wording review page).
- policyIdentity.test.ts: two new tests on the curated demo tenant, as R1 reproduced it. (1) group-assigned admins + staff policies, listed/reversed × neutral/misleading names → held, no update, no create, not executable, untracked. The premise asserts the adjust path, so it cannot pass vacuously; a first version on the uncurated tenant scored the goal `absent`, took the create path, and failed on its premise. (2) Beside an All users policy the admins-group policy is never the target. No existing assertion removed or loosened.

### Checks (S5)
| Check | Command | Exit | Result |
|---|---|---|---|
| Registry | `node scripts/compile-implementation-content.mjs --registry src/content/implementation/registry.generated.json` (`registry.txt`) | 0 | 44 packages; only the two changed sentences differ |
| Typecheck | `npx tsc --noEmit` (`tsc-1` … `tsc-4`) | 0 | no errors |
| Focused #1 | policyIdentity, tracking.drift, foundationA/B/C, policyTruth, planVariants, mfaAuthContentSpecs, planLanes, planLanesHolds, operations, resolvePolicy, content, coverage, tracking (`focused-1.txt`) | 1 | 302 · 301 pass · 1 fail (orphan list, explained above) |
| Focused #2/#3 | policyIdentity + content (`focused-2/3.txt`) | 1 / 0 | #2 17/18 (uncurated premise, test rebuilt); #3 18/18 |
| Focused #4 | tracking.drift, foundationA/B/C, policyTruth, planVariants, mfaAuth/sessionAdmin specs, planLanes(+Holds), packageState, operations, resolvePolicy, coverage, tracking, guestsCoverage, `src/content/implementation/*.test.ts` (`focused-4.txt`) | 0 | 400 pass · 0 fail |
| Full #1 (ae9d246) | `npm test` (`full-test.txt`) | 1 | 2596 · 2588 pass · 6 fail (blockedReason, readinessGate ×2, stepSnapshots, footer, implementationRegion — see 03e6340) · 2 skipped |
| Focused #5 | the six failing files + policyIdentity, planVariants, content, tracking.drift, foundationB (`focused-5.txt`) | 0 | 136 pass · 0 fail |
| Full #2 (03e6340) | `npm test` (`full-test-2.txt`, 23:22–23:25) | 0 | 2596 tests · 2594 pass · 0 fail · 0 cancelled · 2 skipped (Learn-link external health; HUGE=1 — same as S0) · 192 s |
