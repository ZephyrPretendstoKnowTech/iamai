...
|---|---|---|---|---|
| C01 | PENDING (not reproduced in S0) | NOT VERIFIED | No existing test covers separate admin/internal-user policies, renamed policies or reversed input order (resolvePolicy.test.ts test list). Candidate paths, unproven: tracking.ts:292-300 single-member fingerprint takes the first `strong` coverage candidate; coverage.ts:684-700 candidate order follows input; goalIdentity.ts:224 pin-time variant tie-break by exclusion count then `name.localeCompare`; generate.ts:903-916 goal map vs signature fallback | S1: roadmap/fixtures buildFixture Spec is count-based (`policies: number`), so the admin+internal-user+session shape needs a hand-built snapshot through coverage → generate → tracking; then rename and reverse order. Not attempted in S0 (S1-sized) |
