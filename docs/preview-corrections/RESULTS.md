...
|---|---|---|---|---|
| C01 | PENDING (not reproduced in S0) | NOT VERIFIED | No existing test covers separate admin/internal-user policies, renamed policies or reversed input order (resolvePolicy.test.ts test list). Candidate paths, unproven: tracking.ts:292-300 single-member fingerprint takes the first `strong` coverage candidate; coverage.ts:684-700 candidate order follows input; goalIdentity.ts:224 pin-time variant tie-break by exclusion count then `name.localeCompare`; generate.ts:903-916 goal map vs signature fallback | S1: build admin+internal-user+session synthetic fixture, rename and reverse order |
...
| C06 | PENDING | NOT VERIFIED | Authored JSON uses typed `{{json:policy.target.*}}` bindings (e.g. intune-enrollment-reauth CONTENT.md:57 `excludeGroups` binding); no literal quoted placeholder strings in CONTENT.md. protocol.ts:199 `maskJsonTemplate` masks to `null` for validation only. Runtime substitution path producing placeholder strings not located in S0 | S3: locate runtime fill in stepPackage.ts / library.ts |
