...
| G3.3 ⚠️ | 4 | (prior R3-1) | Foundation B lifecycle | **Half done, and the remaining half is not what I first said it was.** The duplicate name and the silence are fixed (G3.2): the step names the policy it already owns and says IAMAI does not turn one on. What is left is the **offer** — handing over the change that switches it on — and the obstacle is not a `packageStateOf` state plus 44 package blocks, as recorded on 2026-09-21. Re-read 2026-09-22: `Lifecycle` has four values (`not-deployed`, `report-only`, `ready-to-enforce`, `enforced`) and `tracking.ts` collapses a **disabled** policy into `not-deployed` at one line. The tenant has a fifth state — deployed but switched off — that the product cannot name, and `claimedPolicy` refuses a disabled policy deliberately because of it. Adding it touches the `Lifecycle` type, its content words, `statusWord.ts`, `planState.ts`, the lane engine's observation, the schedule and the exports. That is a Foundation B change, and it wants its own session with CI green behind it. | Owner: a fifth lifecycle value, or leave the step naming the policy and declining to enable it. |
