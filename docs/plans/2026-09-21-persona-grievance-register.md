...
| G-F1 ✅ | 3 | (found 2026-09-21) | `src/roadmap/fixtures` | Fixed 2026-09-22. Every fixture but the demos gave its break-glass accounts `{ kind: 'fido2' }` — no `id`, no `aaGuid`, no `passkeyType` — a record no tenant produces, which the product reads as unreadable (correctly). They now carry the shape a prepared account gets: an id, an approved model, device-bound, attested. `s-prereq-break-glass` reaches Completed on the shipped tenants and the thirteen steps behind it name their next real prerequisite. 70 step snapshots moved; one new Plan variant. | Done. |
...
| G3.3 ✅ | 4 | (prior R3-1) | `operations.ts`, `stepBody.ts`, `stepExport.ts` | **Closed 2026-09-22, without the fifth lifecycle value.** A policy the tenant has switched off is not a policy to create — it exists, and creating one makes the second policy the step can never finish from — so `switched-off` is now its own unavailable reason. The step names the policy and says to turn it back on in Entra; the portal, PowerShell and JSON tabs inspect the policy that is there instead of walking the reader through **New policy**; and the AI brief, the channel most likely to be pasted into an assistant, carries the inspection lines rather than the create procedure. **Safety:** turning a policy on enforces, so an unmet readiness threshold outranks it and the step says that instead — asserted over the gated steps. `missing-object` is untouched: on 24 of 26 steps it renders the same procedure and is right to, because there the policy genuinely is not there. **Left undone, deliberately:** the row still reads `Not deployed` on a policy that is deployed. That is the fifth `Lifecycle` value, and the owner's call is to add it only if the wording proves to mislead somebody, rather than on the theory that it might. | Done, bar the row's one word. |
...
| G4.3 ✅ | 4 | Nadia | enforce checklist | **Fixed, and it was three small things, not the plumbing I first diagnosed.** (1) The board could not name a Cleanup row at all — `titleOf` resolved step ids only, so the drill reached the page as the bare words "Prerequisite on hold"; `cleanupTitleOf` already existed and is now exported and used, which fixes the row's reason line and every prerequisite tile at once. (2) A policy being created today is not enforcing today, so the drill is not among that step's blockers — `enforceWaits` carries it from the board beside them. (3) The conditions are authored as an indented list under a numbered item, so the number landed alone and rendered as an instruction reading "3.". On `demo-week2`'s token-protection step the checklist now reads **"Verify Emergency Access is not finished yet, and this policy is part of it."** above the three authored conditions, which stay. | Done. |
...
| G5.22 ✅ | 3 | Marcus | `s-prereq-break-glass` at `done` | **Owner decision 2026-09-22: option A.** The procedures stay — `emergencyAccountTasks.ts` keeps them so that "with none needing any, every change stays available as a reference" — and they are typeset as commands, so a step reading Completed drew three task blocks of 8, 9 and 10 imperative lines and a reader took the lot for work remaining. Every word is kept; what changed is the default. On a step the board reads Completed or Deferred the block opens **closed**, under "Reference: how each of these changes is made". Never while printing, and never on a step still open. | Done. |
