...
| G5.5 ⚠️ PART | 3 | Priya | `s-prereq-exclusion-group` | **Guard added; the reported case not located.** A guard is in place so the tile cannot claim "Membership verified" when no emergency accounts are selected, and the same tile now says "Not checked yet" where no membership check ran at all. Neither changes the reported tenant: its mapping holds two break-glass accounts and the member rules return pass, so something upstream supplies a group entry the `groups` map does not contain, and I did not find it. The underlying rule is blocker-severity, so flipping its outcome would move gating and was not done blind. | Find why the member rules pass on a snapshot whose `groups` is empty. |
...
| G5.11 ⚠️ | 3 | Sam | board | **Open, and understood.** `197 guests` vs `Guest Directory · 225 guests`, `51 admins` vs `60 admins`, on one board. Both pairs are derivable: the smaller of each is the population with sign-in evidence, the larger the directory count. Neither number is wrong and neither says which it is. The fix is a disclosure on the two tiles, not a change to either figure. | Each figure says which population it counts. |
| G5.12 ⚠️ | 3 | Sam | headline population | **Open.** `4,169 active people` became `4,167` on clock +1 day and then froze through +10, so it is not a sliding window. Two people left the active set on the first day and the cause was not established in this pass. | Establish why two people leave the active population on day one. |
| G5.13 ⚠️ | 2 | Marcus | `s-goal-device-registration-mfa` | **Open, same shape as G5.11.** Three totals on one step — "246 active people", "covers 283 enabled", "209 of 279 people" — each derivable, none explained. The readiness denominator differs from the step's own population because it counts the campaign cohort. A disclosure, not a recount. | Each figure says which population it counts. |
