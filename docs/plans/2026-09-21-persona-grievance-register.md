...
| G1.5 | 4 | Jordan | board, `midflight` | A step rendering `Ready` / `Accounts and identity · Verified` is cited by three other steps as an unfinished prerequisite. | A prerequisite's cited state matches the step's own state. |
...
| G1.7 | 3 | Marcus, Nadia, Sam, Priya | `s-check-dormant-accounts`, `s-prereq-service-accounts-group`, gates | *"Review each account IAMAI lists with its owner"* / *"as IAMAI lists"* — the list exists only in the `ai` channel. `s-shared-devices` names its account inline, so the product can do it. | Where the product names a set, the set renders where the instruction is. |
...
| G5.22 | 3 | Marcus | `s-prereq-break-glass` at `done` | A Completed step still renders ten imperatives beginning *"Keep your working administrator session open"* and *"sign in as **the account you are preparing**"*, after *"No selected account currently needs an approved passkey."* Never names an account. | Completed steps stop commanding. |
