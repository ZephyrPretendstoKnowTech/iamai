...
The owner's step-by-step walk of the Plan, 1.1 to 5.1 (steps after 5.1 are frozen until these are right). Each section is audited line by line on the owner's tenant, the owner approves items, and approved items land here. Status runs approved → fixed → verified (verified = seen on screen on the owner's tenant in Claude in Chrome). Items the owner turned down are not listed.

**State on 2026-09-24:** sections 1–4 are live (main 3d4a4795). A final Chrome audit of 1.1–4.4 on the owner's tenant came back clean. Items about states the owner's tenant does not show (for example 3.7, 4.5, 4.6, reopened answers, picked service accounts) are live but were checked in the test harness only.
...
| # | Item | Status |
|---|---|---|
| 1 | Every rail reads milestone → bar → instruction → controls | verified |
| 2 | 1.2's rail names what's left once a group is chosen | verified |
| 3 | Milestone headline is words, never a date or lane word | verified |
| 4 | Impact: 1.2 "N policies", 1.3 "N people", 1.4 "2 accounts" | verified |
| 6 | 1.2 membership card reads the group IAMAI holds (no "Could not verify") | verified |
| 7 | 1.2 exclusions task: plain procedure before a group is chosen | verified |
| 8 | 1.2 PowerShell and JSON tabs removed | verified |
| 9 | 1.2 filler lines removed; membership task is the add procedure | verified |
| 10 | 1.2 empty card: "No group selected" plus the create pointer | verified |
| 11 | 1.3 tasks state their values in every state; review task removed | verified |
| 12 | 1.3 Methodology removed | verified |
| 13 | 1.3 PowerShell and JSON tabs removed | verified |
| 14 | 1.3 rail drawn | verified |
| 15 | 1.3 approved models inside the rail | verified |
| 16 | 1.4 uses the shared footer, no Close | verified |
| 17 | 1.4 step-type label and Source checked date | verified |
| 18 | 1.4 Configuration card removed | verified |
| 19 | 1.4 filler lines removed | verified |
| 20 | 1.4 PowerShell and JSON tabs removed | verified |
| — | Rail colouring on all four steps | verified |
| — | About, Completion Criteria and 1.4 milestone rewrites, word for word | verified |
...
**Live since 2026-09-24,** and Chrome-verified on the owner's tenant for every card the tenant shows. Follow-ups from the owner's visual check, also live:
- Approve looks disabled when it is.
- The office line reads "Answering Not in Entra yet leaves Define the Trusted Network for you to do: add your office's public IP addresses to Entra."
- 2.3's counts and 2.1's service counts count only the plan's people.
- 2.2's None lines show only when accounts were found.
- 2.1's mail line matches 4.x: "…Block Legacy Authentication waits until each moves to a supported mail route."

### All three steps
| # | Item | Status |
|---|---|---|
| 8 | Approve answers sits in the rail's controls slot, under the instruction (StepActionColumn), like every other step's controls | fixed |
| 9 | Approve is enabled only when an answer differs from what is saved, or a card still holds an unsaved suggestion | fixed |
| 10 | Approve disabled by an empty pick-list says why under it: "Pick at least one account, or choose None." / "Pick at least one location, or choose another answer." | fixed |
| 11 | Card tag follows the draft: "Suggested" only while the answer is the untouched unsaved suggestion; "Approved" only while it equals the saved answer; otherwise "Not approved yet" | fixed |
| 12 | "Not sure? Keep the suggestion. You can change it any time." only while a card reads Suggested; never on paper | fixed |
| 13 | Every successful Approve opens the next step in order (2.1 → 2.2 → 2.3 → 3.1); today it moves on only after 2.1 | fixed |
| 14 | Evidence in two shapes only. Sign-ins: "{n} people signed in to X in the last 30 days." / "No X sign-ins in the last 30 days." Accounts: "{n} accounts look like X." / "No account looks like X." No "Today:" line, no "The baseline's recommendation.", no "A default, not something the scan saw.": where the scan holds no fact the card shows no evidence line | fixed |
| 15 | Each card whose answer changes the plan carries one short consequence line, "Answering {option} …" (the words are in 27, 31, 35, 44, 54, 57) | fixed |
| 16 | Pick-list options read "None / Pick accounts" (2.1 mail's "Some" goes) and "Pick locations" (2.3's "Trusted locations" goes) | fixed |
...
| 18 | On Hold tiles on other steps lose the note "Answer it in {step}."; the tile keeps "{step} · Waiting on your answers" | fixed |
| 19 | The "Answered in {Direction step}" blocks are deleted from every step (owner rule: no Answered-in blocks) | fixed |
| 24 | "Saving plan…" and "Plan could not be saved. Use Retry Saving above." move to content.json | fixed |
| 25 | Impact on the three Direction steps is the number of plan steps their answers decide: "N steps" | fixed |

### 2.1 Confirm What You Use
| # | Item | Status |
|---|---|---|
| 1 | Delete the device code question and the hold it puts on Block Device Code Sign-in | fixed |
| 2 | Delete the "Sync from on-premises Active Directory" question | fixed |
| 26 | Service evidence: "No {service} sign-ins in the last 30 days." / "{n} people signed in to {service} in the last 30 days.", from sign-in data the scan already reads, 30-day window; no engine-speak | fixed |
| 27 | Service consequence: "Answering No takes {step titles} off your plan." | fixed |
| 28 | Reopened card: "You answered No. {evidence}" | fixed |
| 29 | A reopened card pre-fills the new suggestion, not the old answer | fixed |
| 30 | Mail evidence: "{n} accounts signed in to send email in the last 30 days." / "No email-sending sign-ins in the last 30 days." | fixed |
| 31 | Mail consequence: "Answering Pick accounts makes them service accounts: they leave the people counts, and Block Legacy Authentication leaves them out until each moves to a supported mail route." | fixed |
| 32 | The mail picker leaves out the emergency access accounts and guests | fixed |
| 33 | A suggested sender in the mail picker says why: "Signed in to send email in the last 30 days." | fixed |
| 34 | Partner evidence: "{n} partner or MSP accounts signed in to your tenant in the last 30 days." / "No partner or MSP sign-ins in the last 30 days."; counts accounts, never sign-ins | fixed |
| 35 | Partner consequence: "Answering Yes keeps partner and MSP technicians out of Require MFA for Guests and Block Sign-ins From Countries Not Allowed." | fixed |
| 36 | A saved mail None or partner No reopens when a later scan sees use, as the services already do | fixed |
| 37 | About: "IAMAI filled these in from your tenant. Approve them or correct any that are wrong: a service you don't use takes its policies off your plan, and the accounts and partners you name are kept out of the policies that would stop them." | fixed |

### 2.2 Identify Service and Shared Accounts
| # | Item | Status |
|---|---|---|
| 38 | Label "Service and script accounts" | fixed |
| 39 | Evidence "{n} accounts look like service accounts." / "1 account looks like a service account." / "No account looks like a service account." (the bracket goes) | fixed |
| 40 | Each picked account's chip shows why it was picked: the service candidate's own detection signals; for shared devices "Teams Rooms or shared-device licence" or "signs in only from a Teams device" | fixed |
| 41 | Label "Shared-device accounts"; evidence "{n} accounts look like shared-device accounts." / "1 account looks like a shared-device account." / "No account looks like a shared-device account." | fixed |
| 42 | "A default, not something the scan saw." goes from both cards; the gate reads whether user rows were read, not last-sign-in times | fixed |
| 43 | The people counts follow the saved shared-device answer (detection only while unanswered), as service accounts already do | fixed |
| 44 | Consequences: "Answering None takes Restrict Service Accounts to the Trusted Network off your plan and counts these accounts as people." / "Answering None takes Give Shared Devices Their Own Policy off your plan and counts these accounts as people." | fixed |
| 45 | Create or Correct Service Accounts Group exists only while service accounts are picked, Give Shared Devices Their Own Policy only while shared-device accounts are picked: None adds no steps and no Doesn't-apply rows | fixed |
| 46 | Rail: "Approving these answers sets which accounts count as people and which get a policy of their own." | fixed |
| 47 | About: "Some accounts aren't a person: a mailbox a scanner uses, a script's account, a meeting-room device. IAMAI has picked the ones it found below. Approve or correct them: they leave the people counts, and each kind gets a policy of its own." | fixed |
| 48 | Emergency note: "Your emergency access accounts are already out of the people counts: {names}." | fixed |

### 2.3 Decide How and Where People Sign In
| # | Item | Status |
|---|---|---|
| 4 | Delete the computers option "Hybrid joined" | fixed |
| 5 | Phones: "App protection only" and "Unmanaged" become one option, "No device requirement", the suggestion | fixed |
| 6 | Office note: delete "IAMAI does not read sign-in addresses, so it cannot suggest them."; suggest nothing | fixed |
| 49 | Delete "The baseline's recommendation." from both device cards | fixed |
| 50 | Intune line: "Intune: {consumed} of {seats} licences assigned. Each person who uses a managed computer needs one." Root fix in render.ts pluralise: a verb after a preposition is never bent | fixed |
| 51 | "{n} people signed in from unregistered computers in the last 30 days." / "No sign-ins from unregistered computers in the last 30 days." | fixed |
| 52 | "{n} people signed in from registered computers that aren't joined, in the last 30 days."; its second sentence and the unused key todayRegisteredNone go | fixed |
| 53 | Computer options "Managed (compliant in Intune, or hybrid joined)" and "Unmanaged" | fixed |
| 54 | Computers consequence: "Answering Unmanaged takes Require a Managed Device Outside the Office and Require a Fresh Sign-in for Intune Enrollment off your plan." | fixed |
| 55 | Phone option "Compliant (enrolled in Intune)" | fixed |
| 56 | Phones evidence: "{n} people signed in from a phone in the last 30 days." / "No phone sign-ins in the last 30 days." | fixed |
| 57 | Phones consequence: "Answering Blocked from company data adds Keep Company Data Off Phones: a block policy you build, a report-only week, then a test from an iPhone and an Android phone." With Intune, also: "Answering Compliant adds phones to Require a Managed Device Outside the Office." | fixed |
| 58 | Office label "Office network"; options "Pick locations", "Not in Entra yet", "Everyone works remotely" | fixed |
| 59 | Office evidence names them: "{n} named locations are marked trusted: {names}." | fixed |
| 60 | Office evidence when named locations were not read: the "A default…" line goes, nothing replaces it | fixed |
| 61 | "Everyone works remotely" while a trusted location exists: "{names} is marked trusted in Entra; this answer doesn't use it as your office." And 5.1's "when 1 trusted location exists (now 0)" hold counts Entra's trusted locations, not the answer | fixed |
| 62 | Rail: "Approving these answers sets which device policies your plan holds and which network counts as the office." | fixed |
| 63 | About: "Choose how company computers and phones are managed, and which network is your office. The plan's device and location policies follow these answers." | fixed |
| 64 | Partial-read lines ("In the part of the sign-in records this scan read… There may be more.") go; nothing replaces them | fixed |
| 65 | 2.3's unused AI Info branch (aiGrounding.ts) is deleted | fixed |


## Section 3 Prepare (3.1–3.7): LIVE (2026-09-24). Approved by the owner on 2026-09-23, all items with these changes
**Chrome walk fixes, also live:**
- 3.4 reads the tenant's policies as Graph returns them and agrees with MFA Readiness.
- 3.4's campaign line reads "Set the registration campaign"; the campaign method reads "passkeys".
- 3.2 doesn't count admin tools (Exchange PowerShell) as everyday mail. An admin with a mailbox or Teams licence counts as everyday use too.
- 3.6's criterion holds without a picked location.
- 3.1's Satisfied card reads "Every enabled account has signed in in the last 90 days" ("…, apart from the N you keep"). An account created within 90 days isn't dormant.
- 3.3's About no longer says "now".
...
## Section 4 Turn On MFA for Everyone (4.1–4.6): LIVE (2026-09-24). Approved by the owner on 2026-09-24, with my recommendations and these changes
**Chrome walk fixes, also live:**
- 4.3's roles fold under the create task ("the 46 roles listed below"), and its facts say "admin roles" with no count.
- 4.4's criteria and card name all three delivering policies, and a policy with more than two exclusions names the first and counts the rest.
- 4.1 and 4.2's create names use the tenant's own casing.
- 1.4 reads Ready, with a dated When.
